...
int cEMVWaitAppSel(int TryCnt, EMV_APPLIST List[], int AppNum)
{
	...
	for(iCnt=0; iCnt<iAppCnt && iCnt<MAX_APP_NUM; iCnt++)
	{
		...
		stAppMenuItem[iCnt].vFunc = NULL;
 		sprintf_string((char *)stAppMenuItem[iCnt].szText, "%.16s", stAppList[iCnt].aucAppLabel);
	}
...
ENTERPIN:
	...
	else if (ChkTermPEDMode(PED_EXT_PP))
	{
		...
		case 0x00:
			...
			sprintf_string((char *)pszPlainPin, "%.12s", &szBuff[2]);
...
{
	Gui_ClearScr();
	Gui_ShowMsgBox(GetCurrTitle(),  gl_stTitleAttr, _T("PIN VERIFY OK"), gl_stCenterAttr, GUI_BUTTON_OK, 1, NULL);
}
...
//		sprintf_string((char *)glProcInfo.stTranLog.szCondCode, "06");
		...
		if(ChkIfAmex())
		{
			...
			{
				sprintf_string((char *)glProcInfo.stTranLog.szAuthCode, "Y3");
			} 
			else
			{
				// for AMEX, approval code = Y1 while chip off line apporved.
				sprintf_string((char *)glProcInfo.stTranLog.szAuthCode, "Y1");	
			}		
		}
		else
		{
...
int ViewTVR_TSI(void)
{
	...
	sprintf_string(stBuff[nLine].szLine,"Before TSI=%02X %02X", glEmvStatus.sgTSIOld[0], glEmvStatus.sgTSIOld[1]);
	stBuff[nLine++].stLineAttr = gl_stLeftAttr;

	sprintf_string(stBuff[nLine].szLine,"TVR=%02X %02X %02X %02X %02X",
	...
	sprintf_string(stBuff[nLine].szLine, "IssuAppData=");
	stBuff[nLine++].stLineAttr = gl_stLeftAttr;

	for(iTemp=0; iTemp<glEmvStatus.sgARQCLenOld; iTemp++)
	{	
		sprintf_string(szHex, "%02X", glEmvStatus.sgARQCOld[iTemp]);
	...
	sprintf_string(stBuff[nLine].szLine, "After TSI=%02X %02X", glEmvStatus.sgTSINew[0], glEmvStatus.sgTSINew[1]);
	stBuff[nLine++].stLineAttr = gl_stLeftAttr;
