...
void GetAllSupportFunc(char *pszBuff)
{
	...
	for (ii=0; ii<sizeof(sgFuncMenuItem)/sizeof(sgFuncMenuItem[0]); ii++)
	{
		if (sgFuncMenuItem[ii].szText[0]!=0)
		{
			...
			sprintf_string(pszBuff+strlen(pszBuff), "%lu", (unsigned long)sgFuncMenuItem[ii].nValue);
		}
	}
}
...
int GetCommName(uchar ucCommType, uchar *pszText)
{
	...
	{
	case CT_RS232:
		sprintf_string((char *)pszText, "COM");
		return 0;
	case CT_MODEM:
		sprintf_string((char *)pszText, "MODEM");
	    return 0;
	case CT_TCPIP:
		sprintf_string((char *)pszText, "TCPIP");
	    return 0;
	case CT_WCDMA:
		sprintf_string((char *)pszText, "WCDMA");
		return 0;
	case CT_CDMA:
		sprintf_string((char *)pszText, "CDMA");
		return 0;
	case CT_GPRS:
		sprintf_string((char *)pszText, "GPRS");
		return 0;
	case CT_WIFI:
		sprintf_string((char *)pszText, "WIFI");
	    return 0;
	case CT_DEMO:
		sprintf_string((char *)pszText, "DEMO");
	    return 0;	
	case CT_BLTH:
		sprintf_string((char *)pszText, "BLUETOOTH");
	    return 0;
	default:
		sprintf_string((char *)pszText, "DISABLED");
	    return -1;
	}
}
...
int SetCommType(uchar ucMode)
{
	...
	if (ucMode!=0)
	{
	    ...
	    sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[0].szText);
	    ++iMenuItemNum;
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_MODEM)))
	{
	    if(stDefCommMenu[1].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[1].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_TCPIP)))	// and now is selecting 2nd comm && 1st comm already selected LAN
	{
	    if(stDefCommMenu[2].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[2].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_GPRS)))
	{
	    if(stDefCommMenu[3].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[3].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_CDMA)))
	{
	    if(stDefCommMenu[4].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[4].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_WIFI)))
	{
	    if(stDefCommMenu[5].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[5].szText);
            ++iMenuItemNum;
        }
	}
	if (!(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_RS232))
	{
	    if(stDefCommMenu[6].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[6].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_BLTH)))
	{
	    if(stDefCommMenu[7].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[7].szText);
            ++iMenuItemNum;
        }
	}
		...
		(ucMode!=0 && glSysParam.stTxnCommCfg.ucCommType==CT_WCDMA)))
	{
	    if(stDefCommMenu[8].bVisible)
        {
            ...
            sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[8].szText);
            ++iMenuItemNum;
        }
	}
    if (ucMode==0)
    {
        ...
        sprintf_string(stCommMenu[iMenuItemNum].szText, "%s", stDefCommMenu[9].szText);
        ++iMenuItemNum;
    }
...
int SetCommDetails(uchar mode, uchar *pucCommType)
{
	...
	sprintf_string((char *)szDispBuff, "SETUP ");
...
int SetRs232Param(RS232_PARA *rs232)
{
    ...
    {
        char szAttr[21] = {0};
        sprintf_string(szAttr, "%s%s", stBaudRateMenuItem[iSelected].szText, strchr(rs232->szAttr, ','));
...
int SetModemParam(void)
{
   ...
   //---------------------------------------------------
	...
	sprintf_string((char *)szPrompt, "DIAL WAIT:");
	sprintf_string((char *)szBuff, "OLD:%u(*100ms)", (uint)glSysParam._TxnModemPara.DT1);
	...
	sprintf_string((char *)szPrompt, "PABX PAUSE:");
	sprintf_string((char *)szBuff, "OLD:%u(*100ms)", (uint)glSysParam._TxnModemPara.DT2);
	...
	sprintf_string((char *)szPrompt, "ONE DTMF HOLD:");
	sprintf_string((char *)szBuff, "OLD:%u(*1ms)", (uint)glSysParam._TxnModemPara.HT);
	...
	sprintf_string((char *)szPrompt, "DTMF CODE SPACE:");
	sprintf_string((char *)szBuff, "OLD:%u(*10ms)", (uint)glSysParam._TxnModemPara.WT);
	...
	sprintf_string((char *)szPrompt, "SIGNAL LEVEL:");
	sprintf_string((char *)szBuff, "OLD:%u(0, 1~15)", (uint)glSysParam._TxnPSTNPara.ucSignalLevel);
    while (1)
    {
		Gui_ClearScr();
		if( GUI_OK != Gui_ShowInputBox(GetCurrTitle(), gl_stTitleAttr, szPrompt, gl_stLeftAttr,
...
int GetRemoteIp(const uchar *pszHalfText, uchar bAllowHostName, uchar bAllowNull, void *pstIPAddr)
{
	...
	if(bAllowHostName)
	{
        sprintf_string((char *)szBuff, "%s Host", pszHalfText);
    ...
    {
        sprintf_string((char *)szBuff, "%s IP", pszHalfText);
	...
	sprintf_string((char *)szBuff, "%s PORT", pszHalfText);
...
int SetTcpIpParam_S80(TCPIP_PARA *pstParam)
{
	...
	if (iRet==0)
	{
		sprintf_string((char *)szDispBuff, "DHCP: OK");
		iSelected = 1;
	}
	else
	{
		sprintf_string((char *)szDispBuff, "DHCP: STOPPED");
		iSelected = 0;
	}
		...
		_T("START"), 1, _T("STOP"), 0, USER_OPER_TIMEOUT, &iSelected))
	{
		if(1 == iSelected)
		{
			...
			if (SxxDhcpStart(FALSE, 30)==0)
			{
				...
				sprintf_string(stPageLines[iLines].szLine, "%s:%s", "LOCAL IP", pstParam->szLocalIP);
				...
				sprintf_string(stPageLines[iLines].szLine, "%s:%s", "IP MASK", pstParam->szNetMask);
                stPageLines[iLines].stLineAttr.eAlign = GUI_ALIGN_LEFT;
                stPageLines[iLines].stLineAttr.eFontSize = GUI_FONT_NORMAL;
                stPageLines[iLines].stLineAttr.eStyle = GUI_FONT_STD;
                ++iLines;

				sprintf_string(stPageLines[iLines].szLine, "%s:%s", "GATEWAY IP", pstParam->szGatewayIP);
                stPageLines[iLines].stLineAttr.eAlign = GUI_ALIGN_LEFT;
                stPageLines[iLines].stLineAttr.eFontSize = GUI_FONT_NORMAL;
                stPageLines[iLines].stLineAttr.eStyle = GUI_FONT_STD;
                ++iLines;

				sprintf_string(stPageLines[iLines].szLine, "%s:%s", "DNS", pstParam->szDNSIP);
                stPageLines[iLines].stLineAttr.eAlign = GUI_ALIGN_LEFT;
                stPageLines[iLines].stLineAttr.eFontSize = GUI_FONT_NORMAL;
                stPageLines[iLines].stLineAttr.eStyle = GUI_FONT_STD;
                ++iLines;
...
static int  GetHostDNS(const uchar *pszPrompts, uchar bAllowNull, uchar *pszName)
{
    ...
    sprintf_string((char *)szTemp, "%.50s", pszName);
    Gui_ClearScr();
    ...
    if( bAllowNull && szTemp[0]==0 )
    {
      *pszName = 0;
      ...
      sprintf_string((char *)pszName, "%.50s", szTemp);
    }
    return 0;
}
...
int GetIPAddress(const uchar *pszPrompts, uchar bAllowNull, uchar *pszIPAddress)
{
	...
	sprintf_string((char *)szTemp, "%.15s", pszIPAddress);
	...
	while(1){
        ...
        if( bAllowNull && szTemp[0]==0 )
        {
            *pszIPAddress = 0;
            ...
            sprintf_string((char *)pszIPAddress, "%.15s", szTemp);
        }
        return 0;
	}
	return 0;
...
uchar IsValidIPAddress(const char *pszIPAddr)
{
	...
	sprintf_string(szIp, "%.*s",sizeof(szIp), pszIPAddr); // Modified by Kim_LinHB 2014-8-11 bug507
	...
	for(q=szIp, i=0; i<3; i++)
	{
		...
		sprintf_string(szBuf, "%.*s", (int)(p-q), q);
...
int GetIPPort(const uchar *pszPrompts, uchar bAllowNull, uchar *pszPortNo)
{
	...
	while( 1 )
	{
		sprintf_string((char *)szTemp, "%.5s", pszPortNo);
		Gui_ClearScr();
		...
		{
			sprintf_string((char *)pszPortNo, "%.5s", szTemp);
			break;
		}
		if (bAllowNull)
		{
...
void SetIssuerParam(uchar ucPermission)
{
	...
	while (1)
	{
		...
		sprintf_string((char *)szBuff, "ISSUER: %-8.8s", (char *)glCurIssuer.szName);
		SetCurrTitle(szBuff);
            ...
            while(1){
               ...
               switch(iSelected){
                   ...
                   case 1:
                       ...
                       stInputAttr.nMaxLen = 2;
                       sprintf_string((char *)szBuff2, "%lu", glSysParam.stIssuerList[ucIndex].ulFloorLimit);
                       Gui_ClearScr();
                   ...
                   case 3:
                       if (ChkEdcOption(EDC_TIP_PROCESS))
                       {
                           ...
                           while (1)
                           {
                               ...
                               sprintf_string((char *)szBuff2, "%d", (int)(glSysParam.stIssuerList[ucIndex].ucAdjustPercent%100));
                               Gui_ClearScr();
                   ...
                   case 4:
                       if (ChkEdcOption(EDC_REFERRAL_DIAL))
                       {
                           ...
                           sprintf_string((char *)szBuff2, "%.12s", (char *)glSysParam.stIssuerList[ucIndex].szRefTelNo);
                           Gui_ClearScr();
                           iRet = Gui_ShowInputBox(GetCurrTitle(), gl_stTitleAttr, _T("REFERRAL TEL"), gl_stLeftAttr,
                               szBuff2, gl_stRightAttr, &stInputAttr, USER_OPER_TIMEOUT);
                           if( GUI_OK == iRet )
                           {
                               sprintf_string((char *)glSysParam.stIssuerList[ucIndex].szRefTelNo, "%.12s", (char *)szBuff2);
                           }
                       }
                       else{
                          Gui_ClearScr();
...
void SetAcqParam(uchar ucPermission)
{
	...
	while (1)
	{
		...
		int index = 1;
		sprintf_string((char *)szTitle, "%s", glSysParam.ucAcqNum>9 ? "SELECT ACQ:" : "SELECT ACQUIRER");
        ...
        sprintf_string((char *)szTitle, "ACQ: %-10.10s ", (char *)glCurAcq.szName);
		SetCurrTitle(szTitle);

        iSelected = 0;

	    sprintf_string(stMenuItem[iSelected].szText, "%s", "DISP PARAM");
	    ...
	    sprintf_string(stMenuItem[iSelected].szText, "%s", "INFO");
        stMenuItem[iSelected].bVisible = TRUE;
        ++index;
        stMenuItem[iSelected].nValue = iSelected;
        stMenuItem[iSelected].vFunc = NULL;
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "ACQUIRER NAME");
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "TERMINAL ID");
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "MERCHANT ID");
        stMenuItem[iSelected].bVisible = ucPermission>PM_LOW ? TRUE : FALSE;
        index = ucPermission>PM_LOW ? index + 1 : index;
        stMenuItem[iSelected].nValue = iSelected;
        stMenuItem[iSelected].vFunc = NULL;
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "NII");
        stMenuItem[iSelected].bVisible = ucPermission>PM_LOW ? TRUE : FALSE;
        index = ucPermission>PM_LOW ? index + 1 : index;
        stMenuItem[iSelected].nValue = iSelected;
        stMenuItem[iSelected].vFunc = NULL;
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "BATCH NO.");
        stMenuItem[iSelected].bVisible = ucPermission>PM_LOW ? TRUE : FALSE;
        index = ucPermission>PM_LOW ? index + 1 : index;
        stMenuItem[iSelected].nValue = iSelected;
        stMenuItem[iSelected].vFunc = NULL;
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "COMM. PARAM");
        ...
        sprintf_string(stMenuItem[iSelected].szText, "%s", "PRINTER PARAM");
        ...
        while(1){
            ...
            switch(iSelected){
                ...
                case 6:
                    ...
                    stInputAttr.nMinLen = 1;
                    stInputAttr.nMaxLen = 6;
                    sprintf_string((char *)szBuff, "%lu", glCurAcq.ulCurBatchNo);
                    Gui_ClearScr();
...
int DispAcqPama(void)
{
	...
	sprintf_string(stBuff[ucCnt].szLine, "%-10.10s   %3.3s", glCurAcq.szName, glCurAcq.szNii);
	stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

	sprintf_string(stBuff[ucCnt].szLine, "TID:%s", glCurAcq.szTermID);
	stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

	strcpy(stBuff[ucCnt].szLine, "MID:");
	stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;
	sprintf_string(stBuff[ucCnt].szLine, "%s", glCurAcq.szMerchantID);
	...
	for (ii=0; ii<2; ii++)
	{
		...
		{
			sprintf_string(stBuff[ucCnt].szLine, "SECONDARY COMM");
			stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;
		}
		...
		case CT_TCPIP:
		case CT_WIFI:
			sprintf_string(stBuff[ucCnt].szLine, "Transaction:");
			stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

			if(strlen(glCurAcq.stTxnTCPIPInfo[0].szIP) > 0)
			{
				sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stTxnTCPIPInfo[0].szIP, glCurAcq.stTxnTCPIPInfo[0].szPort);
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}

			if(strlen(glCurAcq.stTxnTCPIPInfo[1].szIP) > 0)
			{
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stTxnTCPIPInfo[1].szIP, glCurAcq.stTxnTCPIPInfo[1].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }

			if(0 == strlen(glCurAcq.stTxnTCPIPInfo[0].szIP) && 0 == strlen(glCurAcq.stTxnTCPIPInfo[1].szIP))
			{
				sprintf_string(stBuff[ucCnt].szLine, "(NULL)");
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}

			sprintf_string(stBuff[ucCnt].szLine, "Settlement:");
			stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

			if(strlen(glCurAcq.stStlTCPIPInfo[0].szIP) > 0)
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stStlTCPIPInfo[0].szIP, glCurAcq.stStlTCPIPInfo[0].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }

            if(strlen(glCurAcq.stStlTCPIPInfo[1].szIP) > 0)
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stStlTCPIPInfo[1].szIP, glCurAcq.stStlTCPIPInfo[1].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }
		...
		case CT_WCDMA:
            ...
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stTxnGPRSInfo[0].szIP, glCurAcq.stTxnGPRSInfo[0].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }

            if(strlen(glCurAcq.stTxnGPRSInfo[1].szIP) > 0)
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stTxnGPRSInfo[1].szIP, glCurAcq.stTxnGPRSInfo[1].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }
            ...
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stStlGPRSInfo[0].szIP, glCurAcq.stStlGPRSInfo[0].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }

            if(strlen(glCurAcq.stStlGPRSInfo[1].szIP) > 0)
            {
                sprintf_string(stBuff[ucCnt].szLine, "%s:%.5s", glCurAcq.stStlGPRSInfo[1].szIP, glCurAcq.stStlGPRSInfo[1].szPort);
                stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
            }
		...
		case CT_MODEM:
			...
			{
				sprintf_string(stBuff[ucCnt].szLine, "%.21s", glCurAcq.TxnTelNo1);
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}
			if(strlen(glCurAcq.TxnTelNo2) > 0)
			{
				sprintf_string(stBuff[ucCnt].szLine, "%.21s", glCurAcq.TxnTelNo2);
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}
			...
			{
				sprintf_string(stBuff[ucCnt].szLine, "%.21s", glCurAcq.StlTelNo1);
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}
			if(strlen(glCurAcq.StlTelNo2) > 0)
			{
				sprintf_string(stBuff[ucCnt].szLine, "%.21s", glCurAcq.StlTelNo2);
				stBuff[ucCnt++].stLineAttr = stLeftAttr_Small;
			}
				  ...
				  ulong ulMin, ulong ulMax, long lOrgValue)
{
	...
	if (lOrgValue>=0)
	{
		sprintf_string((char *)szBuff, "%ld", lOrgValue);
	}
...
int SetTermCurrency(uchar ucPermission)
{
	...
	else
	{
		while(1)
		{
			...
			Gui_ClearScr();
			sprintf_string((char *)szBuff, "%.3s %02X%02X",
		...
		sprintf_string((char *)glSysParam.stEdcInfo.stLocalCurrency.szName, "%.3s", stCurrency.szName);
...
int SetMerchantName(uchar ucPermission)
{
	...
	sprintf_string((char *)szBuff, "%.23s", (char *)glSysParam.stEdcInfo.szMerchantName);
	Gui_ClearScr();
	...
	{
		sprintf_string((char *)glSysParam.stEdcInfo.szMerchantName, "%.23s", (char *)szBuff);
	}
	return 0;
}
int SetMerchantAddr(uchar ucPermission)
{
	...
	sprintf_string((char *)szBuff, "%.46s", (char *)glSysParam.stEdcInfo.szMerchantAddr);
	Gui_ClearScr();
	...
	{
		sprintf_string((char *)glSysParam.stEdcInfo.szMerchantAddr, "%.23s", (char *)szBuff);
	}
...
int SetGetSysTraceNo(uchar ucPermission)
{
	...
	if (ucPermission>PM_LOW)
	{
		...
		sprintf_string((char *)szBuff, "%06ld", glSysCtrl.ulSTAN);
...
int SetGetSysInvoiceNo(uchar ucPermission)
{
	...
	if (ucPermission>PM_LOW)
	{
		...
		sprintf_string((char *)szBuff, "%06ld", glSysCtrl.ulInvoiceNo);
	...
	{
		sprintf_string(szBuff, "TRACE NO\n%06ld", glSysCtrl.ulInvoiceNo);
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, szBuff, gl_stCenterAttr, GUI_BUTTON_CANCEL, USER_OPER_TIMEOUT, NULL);
	}
...
int SetAcceptTimeOut(void)
{
	...
	while( 1 )
	{
		sprintf_string((char *)szBuff, "%d", glSysParam.stEdcInfo.ucAcceptTimeout);
...
int SetNumOfReceipt(void)
{
	...
	while( 1 )
	{
		sprintf_string((char *)szBuff, "%d", iCnt);
		Gui_ClearScr();
...
//		{"LOCKED EDC",			EDC_NOT_KEYBOARD_LOCKED,TRUE,	PM_MEDIUM},
	...
	while( 1 )
	{
        ...
        sprintf_string((char *)stMenuItem[ucCnt].szText, "%.16s", (char *)pstCurOpt[ucCnt].pText);
...
int ChangePassword(void)
{
	...
	for(i = 0; i < sizeof(stDefChgPwdMenuItem)/sizeof(GUI_MENUITEM); ++i){
	    if(stDefChgPwdMenuItem[i].bVisible)
        {
            ...
            sprintf_string(stChgPwdMenuItem[iMenuItemNum].szText, "%s", stDefChgPwdMenuItem[i].szText);
            ++iMenuItemNum;
        }
	}
...
int SetPowerSave(void)
{
	...
	else
	{
		...
		sprintf_string((char *)szBuff, _T("IDLE: SLEEP"));
		if (glSysParam.stEdcInfo.ucIdleShutdown)
		{
			sprintf_string((char *)szBuff, _T("IDLE: SHUTDOWN  "));
		}
		Gui_ClearScr();
		...
		if (glSysParam.stEdcInfo.ucIdleShutdown)
		{
			sprintf_string(szPrompt, "%s[5-60mins]", _T("SHUTDOWN TIMEOUT"));
		}
		else
		{
			sprintf_string(szPrompt, "%s[1-60mins]", _T("PWR SAVE TIMEOUT"));
		}

		while (1)
		{
			Gui_ClearScr();
			sprintf_string((char *)szBuff, "%d", (int)ucTemp);
...
void TestMagicCard(int iTrackNum)
{
	...
	while( 1 )
	{
		sprintf_string((char *)szTitle, "TRACK %d TEST", iTrackNum);
		Gui_ClearScr();
...
int SetWiFiApp(void* pstParam)
{
	...
	{
		sprintf_string((char *)stWiFiAppsMenuItem[ucCnt].szText, "%s", stWiFiApp[ucCnt].Ssid);
