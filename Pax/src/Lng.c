...
int SetLng(const char *name)
{
	...
	fd = open((uchar *)name, O_RDWR);
	if (fd < 0) //打开文件失败
	{
		return -2;
	}

	if (MAX_LNG_FILE_SIZE <= filesize((uchar *)name))
	{
	...
	while (pFile <= (fileBuf+fileSize)) // <=  是因为要包含文件的结束符 '\0'
	{
		...
		{
			//if (*pLineBegin != '#') // 不是注释行
			if (*pLineBegin != '#' && *pLineBegin != '/' && *(pLineBegin+1) != '/' ) // 不是注释行
			{
			  ...
			  size_t size, int (*compar)(const void *, const void *))
{
	...
	//Hided By Kim, cuz this function was called only by function Translate, and the argument value is sizeof() which is greater than zero obviously.
//	if (size > 0) {
		low = 0;
		while (low < high) {
