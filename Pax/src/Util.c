...
int CustomizeAppLibForArabiaLang(uchar bSetToArabia)
{
    ...
    if (bSetToArabia)
    {
		...
		Gui_Init(_RGB_INT_(255, 255, 238), _RGB_INT_(0, 20,255));
...
void InitTransInfo(void)
{
	...
	sprintf_string((char *)glProcInfo.stTranLog.szTipAmount, "%012ld", 0L);
...
int DispVersion(void)
{
	...
	sprintf_string(stBuff[nCnt].szLine, "%s:%-8ld", _T("FREE"), freesize());
	stBuff[nCnt++].stLineAttr = gl_stLeftAttr;

	// Downloaded terminal ID
	sprintf_string(stBuff[nCnt].szLine, "%s:%.8s", _T("TERM ID"), glSysParam.stEdcInfo.szDownLoadTID);
	stBuff[nCnt++].stLineAttr = gl_stLeftAttr;
...
{
	uchar szBuff[10];
	sprintf_string(szBuff, "%-3u", uiLeftTime);
...
void SysHaltInfo(const void *pszDispInfo, ...)
{
	...
	vsprintf_string((char*)szBuff + strlen(szBuff), (char*)pszDispInfo, pVaList);
	va_end(pVaList);
...
void GetTransName(unsigned char *szName)
{
	if( glProcInfo.stTranLog.ucTranType==SALE_OR_AUTH )
	{
		...
		GetEngTime(szTitle);
		sprintf_string(szName, "%.16s", szTitle);
	}
	else
	{
		sprintf_string(szName, "%.16s", _T(glTranConfig[glProcInfo.stTranLog.ucTranType].szLabel));
	}
}
...
int ReadMagCardInfo(void)
{
	...
	if( glEdcMsgPtr->MsgType == MAGCARD_MSG )
	{
		...
		sprintf_string((char *)glProcInfo.szTrack1, "%.*s", LEN_TRACK1, glEdcMsgPtr->MagMsg.track1);
		sprintf_string((char *)glProcInfo.szTrack2, "%.*s", LEN_TRACK2, glEdcMsgPtr->MagMsg.track2);
		sprintf_string((char *)glProcInfo.szTrack3, "%.*s", LEN_TRACK3, glEdcMsgPtr->MagMsg.track3);
	}
	else
	{
		glProcInfo.stTranLog.uiEntryMode = MODE_SWIPE_INPUT;
...
int GetPanFromTrack(uchar *pszPAN, uchar *pszExpDate)
{
	...
	{
		sprintf_string((char *)pszExpDate, "0000");
	}
...
void GetDateTime(uchar *pszDateTime)
{
	...
	GetTime(sCurTime);
	sprintf_string((char *)pszDateTime, "%02X%02X%02X%02X%02X%02X%02X",
...
void GetEngTime(uchar *pszCurTime)
{
	...
	if(strcmp(LANGCONFIG, "Arabic") == 0) //added by Kim_LinHB 2014-6-7
		sprintf_string((char *)pszCurTime, "%02X%02X,%s%02X %02X:%02X", (sCurTime[0]>0x80 ? 0x19 : 0x20), sCurTime[0],
				Month[ucMonth], sCurTime[2],  
				sCurTime[3], sCurTime[4]);
	else
		sprintf_string((char *)pszCurTime, "%s%02X,%02X%02X %02X:%02X", Month[ucMonth],
				sCurTime[2], (sCurTime[0]>0x80 ? 0x19 : 0x20),
				sCurTime[0], sCurTime[3], sCurTime[4]);
//	sprintf_string((char *)pszCurTime, "%.3s %02X,%02X  %02X:%02X", Month[ucMonth],
...
void Conv2EngTime(const uchar *pszDateTime, uchar *pszEngTime)
{
	...
	sprintf_string((char *)pszEngTime, "%s %2.2s, %4.4s  %2.2s:%2.2s", Month[ucMonth],
...
void GetCardHolderName(uchar *pszHolderName)
{
	...
	*pszHolderName = 0;
	...
	sprintf_string((char *)pszHolderName, "%.26s", szTempName);
}
...
void GetHolderNameFromTrack1(uchar *pszHolderName)
{
	...
	*pszHolderName = 0;
	...
	sprintf_string((char *)szOrgName, "%.*s", (int)MIN(sizeof(szOrgName)-1, iLen), p);
...
void ConvertHolderName(const uchar *pszOrgName, uchar *pszNormalName)
{
	...
	sprintf_string((char *)pszNormalName, "%s", pszOrgName);
	if( *pszOrgName==0 )
	{
		return;
	}
	...
	if( pszTitle!=NULL )
	{
		sprintf_string(szBuff, "%s ", pszTitle+1);
		PubTrimStr((uchar *)szBuff);
		pszTemp += sprintf_string(pszTemp, "%s ", szBuff);

		sprintf_string(szBuff, "%.*s ", (int)(pszTitle-pszMidName-1), pszMidName+1);
		PubTrimStr((uchar *)szBuff);
		pszTemp += sprintf_string(pszTemp, "%s ", szBuff);
	}
	else
	{
		sprintf_string(szBuff, "%s", pszMidName+1);
		PubTrimStr((uchar *)szBuff);
		pszTemp += sprintf_string(pszTemp, "%s ", szBuff);
	}
	sprintf_string(pszTemp, "%.*s", (int)(pszMidName-(char *)pszOrgName), pszOrgName);
}
...
int SelectAcqForCard(const uchar *psAcqMatchFlag, uchar *pucCardIndex)
{
	...
	for(ucAcqNum=ucCnt=0; ucCnt<glSysParam.ucAcqNum; ucCnt++)
	{
		...
		sprintf_string(stAcqMenuItem[ucAcqNum].szText, "%.10s", glSysParam.stAcqList[ucCnt].szName);
	...
	sprintf_string((char *)szPrompt, _T("SELECT  HOST"));
	
	Gui_BindMenu(szPrompt, gl_stCenterAttr, gl_stLeftAttr, (GUI_MENUITEM *)stAcqMenuItem, &stAcqMenu);
	Gui_ClearScr();
	iMenuNo = 0;
...
int InsertCardProc(void)
{
    ...
    {
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, glProcInfo.stTranLog.szAppPreferName, gl_stCenterAttr, GUI_BUTTON_OK, 1, NULL);
    }
    else if (strlen(glProcInfo.stTranLog.szAppLabel)!=0)
    {
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, glProcInfo.stTranLog.szAppLabel, gl_stCenterAttr, GUI_BUTTON_OK, 1, NULL);
    }
...
//			return ERR_TRAN_FAIL;
		...
		sprintf_string((char *)glProcInfo.stTranLog.szPan, "%.19s", szCardNo);
		bReadPan = TRUE;
	}
	else if ( !bReadTrack2 )
	{
	...
	if( iRet==EMV_OK )
	{
		sprintf_string((char *)glProcInfo.stTranLog.szHolderName, "%.20s", sTemp);
	}
...
int ConfirmPanInfo(void)
{
	...
	if( iIndex>=0 )
	{
		sprintf_string(stBuff[ucLines].szLine, "%.16s", glSysParam.stIssuerNameList[iIndex].szEnglishName);
	}
	else
	{
		ConvIssuerName(glCurIssuer.szName, szIssuerName);
		sprintf_string(stBuff[ucLines].szLine, "%s", szIssuerName);
	}
	stBuff[ucLines++].stLineAttr = gl_stLeftAttr;

	// Modified by Kim_LinHB 9/9/2014 v1.01.0007
	sprintf_string(stBuff[ucLines].szLine, "%.16s", glProcInfo.stTranLog.szHolderName);
	stBuff[ucLines++].stLineAttr = gl_stLeftAttr;
	...
	{
		sprintf_string(stBuff[ucLines].szLine, "EXP DATE:%2.2s/%2.2s", &glProcInfo.stTranLog.szExpDate[2], &glProcInfo.stTranLog.szExpDate[0]);
		stBuff[ucLines++].stLineAttr = gl_stLeftAttr;
	}
...
void ConvIssuerName(const uchar *pszOrgName, uchar *pszOutName)
{
	...
	sprintf_string((char *)pszOutName, "%.10s", pszOrgName);
...
int DetectCardEvent(uchar ucMode)
{
	...
	else if( (ucMode & CARD_SWIPED) && (ucMode & CARD_INSERTED) )
	{
		...
		if( ChkIfEmvEnable() )
		{
			sprintf_string(szPrompt, "%s", _T("SWIPE/INSERT ..."));
		}
		else
		{
			sprintf_string(szPrompt, "%s", _T("PLS SWIPE CARD"));
		}
	}
	else if( (ucMode & CARD_INSERTED)  )
	{
		...
		{
			sprintf_string(szPrompt, "%s", _T("PLS INSERT CARD"));
		}
	}
	else
	{
...
TAG_SELECT_PLAN:
	...
	{
		sprintf_string((char *)stPlanMenuItems[ii].szText, "%s", (char *)glSysParam.stPlanList[ii].szName);
	...
	Gui_BindMenu(_T("SELECT PLAN"), gl_stCenterAttr, gl_stLeftAttr, (GUI_MENUITEM *)stPlanMenuItems, &stPlanMenu);
...
int DisplayInputAmount(gui_callbacktype_t type, void *data, int *dataLen)
{
    if(GUI_CALLBACK_UPDATE_TEXT == type && data && dataLen){
        ...
        if(0 == amount->pStr[0] || IsNumStr(amount->pStr))
        {
            ...
            sprintf_string(szPrefix, "%.3s", glSysParam.stEdcInfo.stLocalCurrency.szName);
...
// 	sprintf_string((char *)szCurrName, "%.3s%1.1s", glSysParam.stEdcInfo.szCurrencyName, &glSysParam.stEdcInfo.ucCurrencySymbol);
...
uchar ValidAdjustAmount(const uchar *pszBaseAmt, uchar *pszTotalAmt)
{
	...
	sprintf_string((char *)szAdjRate, "%ld", (ulong)(glCurIssuer.ucAdjustPercent+100));
...
void GetDispAmount(uchar *pszAmount, uchar *pszDispBuff)
{
	...
	if(pszDispBuff){
		sprintf_string(pszDispBuff, "%.21s", szOutAmt);
	}
}
...
{
	sprintf_string(glProcInfo.stTranLog.szPan, "%.19s", (char *)pszUsrMsg);
...
int GetExpiry(void)
{
	...
	if( !ChkIssuerOption(ISSUER_EN_EXPIRY) )
	{
		sprintf_string((char *)glProcInfo.stTranLog.szExpDate, "1111");
		return 0;
	}
	...
	while( 1 )
	{
		...
		sprintf_string((char *)glProcInfo.stTranLog.szExpDate, "%.2s%.2s", &szExpDate[2], szExpDate);
		iRet = ValidCardExpiry();
		if( iRet==0 )
		{
			break;
...
int GetDescriptor(void)
{
	...
	if( bInputDesc )
	{
		...
		for(ucCnt=0; ucCnt<ucTotal; ucCnt++)
		{
			...
			sprintf_string(szDesc + strlen(szDesc), "%.21s\n", glSysParam.stDescList[ucDesc].szText);
		}
...
void DispPPPedErrMsg(uchar ucErrCode)
{
	...
	default:
		sprintf_string(szErrMsg, "%s:%02X", _T("PED ERROR"), ucErrCode);
	    break;
	}
...
void DispPciErrMsg(int iErrCode)
{
	...
	case PED_RET_ERR_NO_KEY:
		sprintf_string(szDispBuff, _T("KEY MISSING"));
		break;

	case PED_RET_ERR_KEYIDX_ERR:
		sprintf_string(szDispBuff, _T("INVALID KEY INDEX"));
		break;

	case PED_RET_ERR_DERIVE_ERR:
		sprintf_string(szDispBuff, _T("INVALID KEY LEVEL"));
	    break;

	case PED_RET_ERR_CHECK_KEY_FAIL:
	case PED_RET_ERR_KCV_CHECK_FAIL:
		sprintf_string(szDispBuff, _T("CHECK KEY FAIL"));
	    break;

	case PED_RET_ERR_NO_PIN_INPUT:
		sprintf_string(szDispBuff, _T("PIN BYPASS"));
	    break;

	case PED_RET_ERR_INPUT_CANCEL:
	case PED_RET_ERR_INPUT_TIMEOUT:
		sprintf_string(szDispBuff, _T("INPUT CANCELLED"));
	    break;

	case PED_RET_ERR_WAIT_INTERVAL:
		sprintf_string(szDispBuff, _T("PLS TRY AGAIN LATER"));
	    break;

	case PED_RET_ERR_CHECK_MODE_ERR:
		sprintf_string(szDispBuff, _T("INVALID MODE"));
	    break;

	case PED_RET_ERR_NO_RIGHT_USE:
	case PED_RET_ERR_NEED_ADMIN:
		sprintf_string(szDispBuff, _T("PED ACCESS DENIALED"));
	    break;

	case PED_RET_ERR_KEY_TYPE_ERR:
	case PED_RET_ERR_SRCKEY_TYPE_ERR:
		sprintf_string(szDispBuff, _T("INVALID KEY TYPE"));
	    break;

	case PED_RET_ERR_EXPLEN_ERR:
		sprintf_string(szDispBuff, _T("INVALID PARA"));
	    break;

	case PED_RET_ERR_DSTKEY_IDX_ERR:
		sprintf_string(szDispBuff, _T("INVALID DST INDEX"));
	    break;

	case PED_RET_ERR_SRCKEY_IDX_ERR:
		sprintf_string(szDispBuff, _T("INVALID SRC INDEX"));
	    break;

	case PED_RET_ERR_KEY_LEN_ERR:
		sprintf_string(szDispBuff, _T("INVALID KEY LENGTH"));
	    break;

	case PED_RET_ERR_NO_ICC:
		sprintf_string(szDispBuff, _T("CARD NOT READY"));
	    break;

	case PED_RET_ERR_ICC_NO_INIT:
		sprintf_string(szDispBuff, _T("CARD NOT INIT"));
	    break;

	case PED_RET_ERR_GROUP_IDX_ERR:
		sprintf_string(szDispBuff, _T("INVALID GROUP INDEX"));
	    break;
	...
	case PED_RET_ERR_LOCKED:
		sprintf_string(szDispBuff, _T("PED LOCKED"));
	    break;

	case PED_RET_ERROR:
		sprintf_string(szDispBuff, _T("PED GENERAL ERR"));
	    break;
	...
	default:
		sprintf_string(szDispBuff, "%s:%d", _T("PED ERROR"), iErrCode);
	    break;
	}
...
void MaskPan(const uchar *pszInPan, uchar *pszOutPan)
{
	...
	sprintf_string((char *)pszOutPan, "%.*s", LEN_PAN, szBuff);
}
...
void Get8583ErrMsg(uchar bPackErr, int iErrCode, uchar *pszErrMsg)
{
	...
	if( bPackErr )
	{
		if( iErrCode<-2000 )
		{
			sprintf_string((char *)pszErrMsg, "BIT %d DEF ERR", -iErrCode-2000);
		}
		else if( iErrCode<-1000 )
		{
			sprintf_string((char *)pszErrMsg, "BIT %d PACK ERR", -iErrCode-1000);
		}
		else
		{
			sprintf_string((char *)pszErrMsg, "PACK HEADER ERR");
		}
	}
	else
	{
		...
		{
			sprintf_string((char *)pszErrMsg, "UNPACK %d ERR", -iErrCode-1000);
		}
		else if( iErrCode==-1000 )
		{
			sprintf_string((char *)pszErrMsg, "DATA LENGTH ERR");
		}
		else
		{
			sprintf_string((char *)pszErrMsg, "UNPACK HEAD ERR");
		}
	}
}
...
int GetPreAuthCode(void)
{
	...
	sprintf_string((char *)glProcInfo.stTranLog.szAuthCode, "%-*s", LEN_AUTH_CODE, szBuff);
...
int  GetStateText(ushort uiStatus, uchar *pszStaText)
{
	if(!pszStaText)
		return -1;
	sprintf_string((char *)pszStaText,"Normal");
	if( uiStatus & TS_VOID)
	{
		sprintf_string((char *)pszStaText,"VOIDED");
		return 1;
	}
	else if( uiStatus & TS_ADJ)
	{
		sprintf_string((char *)pszStaText, "ADJUSTED");
	}
	else if( uiStatus & TS_NOSEND)
	{
		sprintf_string((char *)pszStaText, "NOT_SEND");
		return 0;
	}
	return 0;
}
...
int SelectAcq(uchar bAllowSelAll, const uchar *pszTitle, uchar *pucAcqIndex)
{
	...
	if( bAllowSelAll )
	{
		sprintf_string((char *)stAcqMenuItem[iMenuNum].szText, "SETTLE ALL");
	...
	for(ucCnt=0; ucCnt<glSysParam.ucAcqNum; ucCnt++)
	{
		sprintf_string((char *)stAcqMenuItem[iMenuNum].szText, "%3.3s %.10s",
	...
	Gui_BindMenu(pszTitle, gl_stCenterAttr, gl_stLeftAttr, (GUI_MENUITEM *)stAcqMenuItem, &stAcqMenu);
	Gui_ClearScr();
	iSelMenu = 0;
...
int SelectIssuer(uchar *pucIssuerIndex)
{
	...
	{
		sprintf_string((char *)stIssuerMenuItem[ucCnt].szText, "%.10s", glSysParam.stIssuerList[ucCnt].szName);
	...
	Gui_BindMenu("SELECT ISSUER", gl_stCenterAttr, gl_stLeftAttr, (GUI_MENUITEM *)stIssuerMenuItem, &stIssuerMenu);
...
int DispTransTotal(uchar bShowVoidTrans)
{
	...
	sprintf_string((char *)stBuff[ucCnt].szLine, "%s%12d", _T("SALE"),    glTransTotal.uiSaleCnt);
	...
	sprintf_string((char *)stBuff[ucCnt].szLine, "%s%10d", _T("REFUND"),   glTransTotal.uiRefundCnt);
	stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

	sprintf_string((char *)szDispAmt, "%.12s", glTransTotal.szRefundAmt);
	szDispAmt[0] = 'D';
	...
	if( bShowVoidTrans )
	{
		sprintf_string((char *)stBuff[ucCnt].szLine, "%s%5d", _T("VOIDED SALE"), glTransTotal.uiVoidSaleCnt);
		stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;

		sprintf_string((char *)szDispAmt, "%.12s", glTransTotal.szVoidSaleAmt);
		szDispAmt[0] = 'D';
		GetDispAmount(szDispAmt, stBuff[ucCnt].szLine);
		stBuff[ucCnt++].stLineAttr = st_Small_Left;

		sprintf_string((char *)stBuff[ucCnt].szLine, "%s%5d", _T("VOIDED REFD"), glTransTotal.uiVoidRefundCnt);
		stBuff[ucCnt++].stLineAttr = gl_stLeftAttr;
...
void AdjustLcd(void)
{
	...
	while( 1 )
	{
		...
		sprintf_string((char *)szBuff, _T("STEP = [%d]"), glSysParam.stEdcInfo.ucScrGray);
...
void DispHostRspMsg(uchar *pszRspCode, HOST_ERR_MSG *pstMsgArray)
{
	...
	sprintf_string(szDispMsg, _T(pstMsgArray[iCnt].szMsg));
	...
	if( ChkIfBea() )
	{
		...
		{
			sprintf_string(szDispMsg, _T("PLS CALL BANK"));
		}
	}

	sprintf_string(szDispMsg + strlen(szDispMsg), "\n%.2s", pszRspCode);
...
int FindCurrency(const uchar *pszCurrencyNameCode, CURRENCY_CONFIG *pstCurrency)
{
	...
	for (iCnt=0; glCurrency[iCnt].szName[0]!=0; iCnt++)
	{
		...
		{
			sprintf_string((char *)sBuff, "0%.3s", pszCurrencyNameCode);
...
{
	uchar	szBuff[64 + 30];
	sprintf_string((char *)szBuff, "%02d:%d*%d %s%s\n%02d:%d*%d %s%s %d",
...
int EnumSysFonts(void)
{
	...
	for (ii=0; ii<iRet; ii++)
	{
		sprintf_string((char *)stInfoListItem[ii].szText, "%02d-%02dx%02d,%d,%d",
...
int SxxWriteKey(uchar ucSrcKeyIdx, const uchar *psKeyBCD, uchar ucKeyLen, uchar ucDstKeyId, uchar ucDstKeyType, const uchar *psKCV)
{
	...
	else
	{
		...
		stKcvInfoIn.szCheckBuf[0] = 4;
		memcpy(stKcvInfoIn.szCheckBuf+1, psKCV, 4);
	}
...
void SetCurrTitle(const unsigned char *pszTitle)
{
	if (pszTitle)
	{
		sprintf_string(gl_szCurrTitle, "%.*s", sizeof(gl_szCurrTitle), pszTitle);
	}
}
...
void MergeIpAddress(const unsigned char sub[4], unsigned char *Ip)
{
    if(Ip)
    {
        sprintf_string(Ip, "%d.%d.%d.%d", sub[0], sub[1], sub[2], sub[3]);
    }
}
...
const char *GetCurSignPrefix(uchar ucAcqKey)
{
    ...
    if(ACQ_ALL == ucAcqKey)
        sprintf_string(szPath, "SIG-");
    else
        sprintf_string(szPath, "SIG-%d-%d", ucAcqKey, glCurAcq.ulCurBatchNo);
    return szPath;
}
char HasE_Signature()
{
...
void DoE_Signature()
{
    ...
    { // touch screen, focus to get the E signature
		int iRet;
		sprintf_string(glProcInfo.stTranLog.szSignPath, "%s-%d", GetCurSignPrefix(glCurAcq.ucKey), glProcInfo.stTranLog.ulInvoiceNo);
        Gui_ClearScr();
