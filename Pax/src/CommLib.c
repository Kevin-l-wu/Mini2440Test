...
static int TcpTxd(const uchar *psTxdData, ushort uiDataLen, ushort uiTimeOutSec);
...
static int WIFITxd(const uchar *psTxdData, ushort uiDataLen, ushort uiTimeOutSec);
static int WIFIRxd(uchar *psRxdData, ushort uiExpLen, ushort uiTimeOutSec, ushort *puiOutLen);
static int WIFIOnHook(uchar bReleaseAll);
...
int CommTxd(const uchar *psTxdData, ushort uiDataLen, ushort uiTimeOutSec)
{
	...
	case CT_WIFI:
		iRet = WIFITxd(psTxdData, uiDataLen, uiTimeOutSec);
		break;
	case CT_TCPIP:
...
int CommRxd(uchar *psRxdData, ushort uiExpLen, ushort uiTimeOutSec, ushort *puiOutLen)
{
	...
	case CT_WIFI:
		iRet = WIFIRxd(psRxdData, uiExpLen, uiTimeOutSec, puiOutLen);
		break;
...
void CommGetErrMsg(int iErrCode, COMM_ERR_MSG *pstCommErrMsg)
{
	...
	{
		sprintf_string((char *)pstCommErrMsg->szMsg, "COMM ERR:%d", iErrCode);
		GetAllErrMsg(iErrCode, sg_stSxxErrMsg, pstCommErrMsg);
		return;
	}

	sprintf_string((char *)pstCommErrMsg->szMsg, "COMM ERR:%04X", iErrCode);
...
void GetAllErrMsg(int iErrCode, const ERR_INFO *pstInfo, COMM_ERR_MSG *pstCommErrMsg)
{
	...
	for(iCnt=0; pstInfo[iCnt].iErrCode!=0; iCnt++)
	{
		...
		{
			sprintf_string((char *)pstCommErrMsg->szMsg, "%.16s", pstInfo[iCnt].szMsg);
			break;
		}
	}
}
...
// 发送数据
// send data
int TcpTxd(const uchar *psTxdData, ushort uiDataLen, ushort uiTimeOutSec)
{
...
TAG_RETRY_IP:
    ...
    if (ucSecondIP)
    {
        if(!ChkIfValidIp(sg_stCurCfg.stWifiPara.stHost2.szIP)){
           ...
           if(RET_OK == iRet)
               iRet = SxxTcpConnect(szIpFromDNS, (short)atoi((char *)sg_stCurCfg.stWifiPara.stHost2.szPort), 8);
       }
       else
           iRet = SxxTcpConnect(sg_stCurCfg.stWifiPara.stHost2.szIP, (short)atoi((char *)sg_stCurCfg.stWifiPara.stHost2.szPort), 8);
    }
    else
    {
        if(!ChkIfValidIp(sg_stCurCfg.stWifiPara.stHost1.szIP)){
             ...
             if(RET_OK == iRet){
                 iRet = SxxTcpConnect(szIpFromDNS, (short)atoi((char *)sg_stCurCfg.stWifiPara.stHost1.szPort), 8);
             }
         }
         else{
             iRet = SxxTcpConnect(sg_stCurCfg.stWifiPara.stHost1.szIP, (short)atoi((char *)sg_stCurCfg.stWifiPara.stHost1.szPort), 8);
         }
    }
...
static ushort PSTNSendATCmdBeforeDial(void)
{
    ...
    {
        sprintf_string((char *)szCmd, "ATS91=%d", (int)sg_stCurCfg.stPSTNPara.ucSignalLevel);
