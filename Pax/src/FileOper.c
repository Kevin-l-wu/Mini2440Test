...
int SaveRevInfo(uchar bSaveRevInfo)
{
	...
	if( bSaveRevInfo )	// Save reversal data
	{
		...
		sprintf_string((char *)pstRevPack->szPan,     "%.*s", LEN_PAN,      glProcInfo.stTranLog.szPan);
		sprintf_string((char *)pstRevPack->szExpDate, "%.*s", LEN_EXP_DATE, glProcInfo.stTranLog.szExpDate);
		iRet = PubFileWrite(FILE_SYS_CTRL,
...
int SaveEmvErrLog(void)
{
	...
	sprintf_string((char *)stErrLog.szPAN, "%.19s", glProcInfo.stTranLog.szPan);
	stErrLog.ucPANSeqNo = glProcInfo.stTranLog.bPanSeqOK ? glProcInfo.stTranLog.ucPanSeqNo : 0xFF;
	sprintf_string((char *)stErrLog.szAmount, "%.12s", glSendPack.szTranAmt);
	sprintf_string((char *)stErrLog.szTipAmt, "%.12s", glSendPack.szExtAmount);
	sprintf_string((char *)stErrLog.szRspCode, "%.2s", glProcInfo.stTranLog.szRspCode);
	GetDateTime(stErrLog.szDateTime);
	sprintf_string((char *)stErrLog.szRRN, "%.12s",    glProcInfo.stTranLog.szRRN);
	sprintf_string((char *)stErrLog.szAuthCode, "%.6s", glProcInfo.stTranLog.szAuthCode);
