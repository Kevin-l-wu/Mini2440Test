...
//	nSetDETData((uchar *)"\xFF\x81\x32", 3, (uchar *)"\x7B\x00", 2);
	...
	if (tTransParam.ucTransType == 0x20)//refund rquired AAC
	{
		nSetDETData("\xDF\x81\x21", 3, (uchar *)"\xFF\xFF\xFF\xFF\xFF", 5);//TAC denial
	}
	else
	{
//		nSetDETData("\xDF\x81\x21", 3, "\x04\x40\x00\x80\x00", 5);
	...
	if (tTransParam.ucTransType == 0x20)//Refund process
	{
	...
	nSetDETData("\xDF\x81\x24", 3, "\x10\x00\x00\x00\x00\x00", 6);
	nSetDETData("\xDF\x81\x26", 3, "\x00\x00\x00\x00\x10\x00", 6);//cvm limit
	...
	nSetDETData("\x9F\x1A", 2, "\x08\x40", 2); //Terminal Country Code
	if(tTransParam.ucTransType == 0x01)
	{
		iRet = nSetDETData("\x9F\x35", 2, "\x14", 1);
	}
	...
	nSetDETData((uchar *)"\x5F\x2A", 2, "\x08\x40", 2);
...
void ClssTxnParamSet_JCB(void)
{
	...
	sprintf_string((char *)aucBuff, "%012ld", tTransParam.ulAmntAuth);
	...
	sprintf_string((char *)aucBuff, "%012ld", tTransParam.ulAmntOther);
...
void ClssTermParamSet_MC(void)//for paypass
{
	...
	{
		nSetDETData("\xDF\x81\x21", 3, (uchar *)"\xFF\xFF\xFF\xFF\xFF\xFF", 5);//TAC denial
	...
	nSetDETData("\xDF\x81\x26", 3, "\x00\x00\x00\x01\x00\x00", 6);//cvm limit
...
// 		{
// #ifdef _LINUX_TERM
// 			snprintf_string((char *)aucBuff, 13, "%012ld", 1000);
// #else
// 			sprintf_string((char *)aucBuff, "%012ld", 1000);
// #endif
// 		}
// 		else
// 		{
// #ifdef _LINUX_TERM
// 			snprintf_string((char *)aucBuff, 13, "%012ld", 20000);
// #else
// 			sprintf_string((char *)aucBuff, "%012ld", 20000);
// #endif
// 		}
// 		vTwoOne_app(aucBuff, 12, aucAmount);
...
// 		memset(aucBuff, 0, sizeof(aucBuff));
// #ifdef _LINUX_TERM
// 		snprintf_string((char *)aucBuff, 13, "%012ld", 30000);
// #else
// 		sprintf_string((char *)aucBuff, "%012ld", 30000);
// #endif
// 		vTwoOne_app(aucBuff, 12, aucAmount);
// 		nSetDETData("\xDF\x81\x26", 3, aucAmount, 6);// Reader CVM Required Limit [12/30/2014 jiangjy]
	...
	// 		{
	// #ifdef _LINUX_TERM
	// 			snprintf_string((char *)aucBuff, 13, "%012ld", 10000);
	// #else
	// 			sprintf_string((char *)aucBuff, "%012ld", 10000);
	// #endif
	// 		}
	// 		vTwoOne_app(aucBuff, 12, aucAmount);
	// 		nSetDETData("\xDF\x81\x26", 3, aucAmount, 6);
...
//	OsLog(LOG_INFO, "Clss_TransProc_DPAS ret = %d", iRet);
	cSendTransDataOutput_DPAS(0x07);
	if (gl_aucUserInterReqData[0] == MI_SEE_PHONE)
	{
		Inter_DisplayMsg(MSG_SEE_PHONE);
...
void Inter_DisplayMsg(unsigned int iMsg)
{
	...
	case MSG_APPROVED:
		...
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("TXN APPROVED"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
		break;
	case MSG_DECLINED:
        SetClssLightStatus(CLSSLIGHTSTATUS_ERROR);
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("TXN DECLINED"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
		break;
	case MSG_APPROVED_SIGN:
        SetClssLightStatus(CLSSLIGHTSTATUS_COMPLETE);
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("TXN APPROVED WITH SIG"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
		break;
	case MSG_OTHER_CARD:
        SetClssLightStatus(CLSSLIGHTSTATUS_ERROR);
...
void vAppDisplayMsgByRet(int nRet)
{
	...
	case CLSS_DECLINE:
	case EMV_DENIAL:
//		Inter_DisplayMsg(Msg_DECLINED);			//modified by kevinliu 2015/10/19
		Inter_DisplayMsg(MSG_DECLINED);
		break;
	case CLSS_USE_CONTACT:
//		Inter_DisplayMsg(Msg_TRY_ANOTHER_INTERFACE);			//modified by kevinliu 2015/10/19
	...
	case ICC_RESET_ERR:
	case ICC_CMD_ERR:
//		Inter_DisplayMsg(Msg_TRY_AGAIN);			//modified by kevinliu 2015/10/19
...
//		Inter_DisplayMsg(Msg_SEE_PHONE);			//modified by kevinliu 2015/10/19
		...
		break;
	default:
//		Inter_DisplayMsg(Msg_END_APPLICATION);			//modified by kevinliu 2015/10/19
...
{  
	if (!ret) 
		return 0;
	else if(ret == CLSS_CVMDECLINE)
	{
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("CL TXN not permitted"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
	}
	...
	else if(ret == EMV_RSP_ERR||ret == EMV_DATA_ERR||ret == CLSS_TERMINATE)
	{
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("TXN TERMINATED"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
	}
	...
	{
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("NOT ACCEPTTED"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
	}
	else if(ret == EMV_DENIAL)
	{
		Gui_ClearScr();
		Gui_ShowMsgBox(GetCurrTitle(), gl_stTitleAttr, _T("TXN DCLINED"), gl_stCenterAttr, GUI_BUTTON_NONE, 0, NULL);
		Beep();
	}
