...
void LoadDefCommPara(void)
{
	...
	sprintf_string((char *)glSysParam._TxnRS232Para.szAttr, "9600,8,n,1");
#ifdef _Dxxx_
	...
	sprintf_string((char *)glSysParam._TxnBlueToothPara.stCommParam.szAttr, "115200,8,n,1");//hdadd // Modified by Kim_LinHB 2014-08-15 v1.01.0004
...
void ResetAllPara(uchar bFirstTime)
{
	...
	if( !bFirstTime )
	{
		...
		sprintf_string((char *)szDownTelNo,   "%.25s", glSysParam.stEdcInfo.szDownTelNo);
		sprintf_string((char *)szDownLoadTID, "%.8s",  glSysParam.stEdcInfo.szDownLoadTID);
	...
	sprintf_string((char *)glSysParam.stEdcInfo.szTMSNii, "000");
	...
	if( !bFirstTime )
	{
		...
		sprintf_string((char *)glSysParam.stEdcInfo.szDownTelNo,   "%.25s", szDownTelNo);
		sprintf_string((char *)glSysParam.stEdcInfo.szDownLoadTID, "%.8s",  szDownLoadTID);
...
int SelectDownloadMode(uchar *pucCommType)
{
	...
	if (!ChkHardware(HWCFG_MODEM, HW_NONE) && stDefCommMenuItem[0].bVisible)
	{
		...
		sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[0].szText);
	    ++iMenuItemNum;
	}
	if (!ChkHardware(HWCFG_LAN, HW_NONE) && stDefCommMenuItem[1].bVisible)
	{
        ...
        sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[1].szText);
        ++iMenuItemNum;
	}
	if (!ChkHardware(HWCFG_GPRS, HW_NONE) && stDefCommMenuItem[2].bVisible)
	{
        ...
        sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[2].szText);
        ++iMenuItemNum;
	}
	if (!ChkHardware(HWCFG_CDMA, HW_NONE) && stDefCommMenuItem[3].bVisible)
	{
        ...
        sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[3].szText);
        ++iMenuItemNum;
	}
	if (!ChkHardware(HWCFG_WCDMA, HW_NONE) && stDefCommMenuItem[11].bVisible)  // added by  Gillian 2015/11/23
	{
	    ...
	    sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[11].szText);
	    ++iMenuItemNum;
	}

    memcpy(&stCommMenuItem[iMenuItemNum], &stDefCommMenuItem[6], sizeof(GUI_MENUITEM));
    sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[6].szText);
    ++iMenuItemNum;
   	...
   	if (!ChkHardware(HWCFG_WIFI, HW_NONE) && stDefCommMenuItem[8].bVisible)	//
	{
         ...
         sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[8].szText);
         ++iMenuItemNum;
	}
    ...
    if (!ChkHardware(HWCFG_MODEM, HW_NONE))
    {
        if(stDefCommMenuItem[4].bVisible)
        {
            ...
            sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[4].szText);
            ++iMenuItemNum;
        }
        ...
        if(stDefCommMenuItem[5].bVisible)
        {
            ...
            sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[5].szText);
            ++iMenuItemNum;
        }
    }

    memcpy(&stCommMenuItem[iMenuItemNum], &stDefCommMenuItem[10], sizeof(GUI_MENUITEM));
    sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[10].szText);
    ++iMenuItemNum;

#ifdef ALLOW_NO_TMS
   	memcpy(&stCommMenuItem[iMenuItemNum], &stDefCommMenuItem[7], sizeof(GUI_MENUITEM));
    sprintf_string(stCommMenuItem[iMenuItemNum].szText, "%s", stDefCommMenuItem[7].szText);
    ++iMenuItemNum;
#endif
...
int NewTmsDownLoad(uchar ucCommType)
{
	...
	sprintf_string((char *)szTermID, "%.8s", glSysParam.stEdcInfo.szDownLoadTID);
	...
	case CT_MODEM:
		...
		sprintf_string((char *)szTelNo, "%.25s.", glSysParam.stEdcInfo.szDownTelNo);
	...
	case CT_WCDMA:
		...
		break;
	case CT_WIFI:
		sprintf_string(stLocalInfo.szLocalIP, "%d.%d.%d.%d", 
		...
		sprintf_string(stLocalInfo.szNetMask, "%d.%d.%d.%d", 
		...
		sprintf_string(stLocalInfo.szGatewayIP, "%d.%d.%d.%d", 
		...
		sprintf_string(stLocalInfo.szDNSIP, "%d.%d.%d.%d", 
...
int OldTmsDownLoad(void)
{
	...
	sprintf_string((char *)glCurAcq.szNii,    "%.3s", glSysParam.stEdcInfo.szTMSNii);
	sprintf_string((char *)glCurAcq.szTermID, "%.8s", glSysParam.stEdcInfo.szDownLoadTID);
...
void UnpackParaEdc(const uchar *psPara)
{
	...
	sprintf_string(glSysParam.stEdcInfo.szMerchantAddr, "%.46s", pstEdc->sMerchantAddr);
	sprintf_string(glSysParam.stEdcInfo.szMerchantName, "%.23s", pstEdc->sMerchantName);
	memcpy(glSysParam.stEdcInfo.szAddlPrompt,   pstEdc->sAddlPrompt,   20);
	sprintf_string(glSysParam.stEdcInfo.stLocalCurrency.szName, "%.3s", pstEdc->sCurrencyName);
...
{
	sprintf_string(ip_out, "%u.%u.%u.%u", ip_in[0],ip_in[1],ip_in[2],ip_in[3]);
}
...
void TransformPort(const uchar * port_in, uchar * port_out)
{
	...
	sprintf_string(port_out, "%d", iPortNum );
}
...
int UnpackInstPara(const uchar *psPara)
{
	...
	sprintf_string((char *)glSysParam.stPlanList[ucNum].szName, "%.7s", pstPlan->sName);
...
int UnpackDescPara(const uchar *psPara)
{
	...
	sprintf_string((char *)glSysParam.stDescList[ucNum].szCode, "%.2s", pstDesc->sCode);
	sprintf_string((char *)glSysParam.stDescList[ucNum].szText, "%.20s", pstDesc->sText);
...
int GetDownLoadTelNo(void)
{
	...
	sprintf_string((char *)szBuff, "%.25s", glSysParam.stEdcInfo.szDownTelNo);
	...
	sprintf_string((char *)glSysParam.stEdcInfo.szDownTelNo, "%.25s", szBuff);
	SaveEdcParam();
...
int GetDownLoadTID(uchar *pszID)
{
	...
	while (1)
	{
		sprintf_string((char *)szBuff, "%.8s", pszID);
		Gui_ClearScr();
	...
	sprintf_string((char *)pszID, "%.8s", szBuff);
	return 0;
}
...
void GetNextAutoDayTime(uchar *pszDateTimeInOut, ushort uiInterval)
{
	...
	sprintf_string((char *)pszDateTimeInOut, "%02d%02d%02d%02d%02d", iYear%100, iMonth,
			iDate, iHour, iMinute);
}
...
int SaveDetail(const uchar *psData)
{
	...
	sprintf_string((char *)szBuf, "%.*s", (int)MIN(psCurPtr-psBack, 9), psBack);
	...
	sprintf_string((char *)szStartNo, "%.*s", (int)MIN(psCurPtr-psBack, 19), psBack);
	sprintf_string((char *)szEndNo,   "%.*s", (int)MIN(psCurPtr-psBack, 19), psCurPtr+1);
...
int SaveCardBin(uchar *psCardBinInOut)
{
	...
	while( *psCurPtr )
	{
		...
		sprintf_string((char *)szChineseName, "%.16s", szBuf);
		...
		sprintf_string((char *)szEnglishName, "%.*s", MAX_CARBIN_NAME_LEN, szBuf);
		...
		sprintf_string((char *)glSysParam.stIssuerNameList[uiIndex].szChineseName, "%.16s", szChineseName);
		sprintf_string((char *)glSysParam.stIssuerNameList[uiIndex].szEnglishName, "%.*s", MAX_CARBIN_NAME_LEN, szEnglishName);
...
void InitEdcParam(void)
{
	...
	GetDefCurrency(&stLocalCurrency);
	sprintf_string((char *)glSysParam.stEdcInfo.szMerchantName, "MERCHANT NAME");
	sprintf_string((char *)glSysParam.stEdcInfo.szMerchantAddr, "MERCHANT ADDR");
	sprintf_string((char *)&glSysParam.stEdcInfo.szDownLoadTID, "00000000");
	...
	sprintf_string((char *)glSysParam.stAcqList[iIAcqCnt].szNii,     "019");

	sprintf_string((char *)glSysParam.stAcqList[iIAcqCnt].szName, "VM_ACQ");
	sprintf_string((char *)glSysParam.stAcqList[iIAcqCnt].szPrgName, "VISA");
	sprintf_string((char *)glSysParam.stAcqList[iIAcqCnt].szTermID,  "00000001");
	sprintf_string((char *)glSysParam.stAcqList[iIAcqCnt].szMerchantID, "123456789012345");
	...
	memcpy(glSysParam.stAcqList[iIAcqCnt].sIssuerKey, "\x01\x02\x03", 3);// visa master other // modified by Kim 20150116 bug611
	++iIAcqCnt;
	...
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName, "VISA");
//	memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
	...
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName, "MASTER");
//	memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
	memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFE\x1E\x10\x00", 4);
	memcpy(glSysParam.stIssuerList[iIssuerCnt].sPanMask, "\x00\xFF\xF0", 3);
	glSysParam.stIssuerList[iIssuerCnt].ulFloorLimit    = 0;
    ...
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName, "AMEX");
//    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFE\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sPanMask, "\x00\xFF\xF0", 3);
    glSysParam.stIssuerList[iIssuerCnt].ulFloorLimit    = 0;
...
//    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
    ...
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName, "DINERS");
//    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFE\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sPanMask, "\x00\xFF\xF0", 3);
    glSysParam.stIssuerList[iIssuerCnt].ulFloorLimit    = 0;
    ...
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
    sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName, "JCB");
//    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFC\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFE\x1E\x10\x00", 4);
    memcpy(glSysParam.stIssuerList[iIssuerCnt].sPanMask, "\x00\xFF\xF0", 3);
    glSysParam.stIssuerList[iIssuerCnt].ulFloorLimit    = 0;
	...
	glSysParam.stIssuerList[iIssuerCnt].ucKey = 0x07;
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szRefTelNo, "86");
	sprintf_string((char *)glSysParam.stIssuerList[iIssuerCnt].szName,     "OTHER");
//	memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption,  "\xFC\x1E\x10\x00", 4);
	memcpy(glSysParam.stIssuerList[iIssuerCnt].sOption, "\xFE\x1E\x10\x00", 4);
	memcpy(glSysParam.stIssuerList[iIssuerCnt].sPanMask, "\x00\xFF\xF0",     3);
	glSysParam.stIssuerList[iIssuerCnt].ulFloorLimit    = 0;
	...
	// set card range
	// VISA
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x40\x00\x00\x00\x00", 5);
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x49\x99\x99\x99\x99", 5);
	...
	//MASTER
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x50\x00\x00\x00\x00", 5);
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x59\x99\x99\x99\x99", 5);
    ...
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x34\x00\x00\x00\x00", 5);
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x34\x99\x99\x99\x99", 5);
    ...
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x37\x00\x00\x00\x00", 5);
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x37\x99\x99\x99\x99", 5);
    ...
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x36\x00\x00\x00\x00", 5);
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x36\x99\x99\x99\x99", 5);
	...
	//JCB
    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x35\x00\x00\x00\x00", 5);
//    memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x39\x99\x99\x99\x99", 5);	//modified by Kevinliu 20160505
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x35\x99\x99\x99\x99", 5);
	...
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeLow,  "\x60\x00\x00\x00\x00", 5);
	memcpy(glSysParam.stCardTable[iCardTableCnt].sPanRangeHigh, "\x99\x99\x99\x99\x99", 5);
	glSysParam.stCardTable[iCardTableCnt].ucIssuerKey = 0x07;
	glSysParam.stCardTable[iCardTableCnt].ucAcqKey    = 0x01;
	glSysParam.stCardTable[iCardTableCnt].ucPanLength = 0;
	glSysParam.stCardTable[iCardTableCnt].ucOption    = 0x02;
	++iCardTableCnt;
