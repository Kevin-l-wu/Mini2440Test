...
int DispHexLine(uchar *pszBuffInOput, uint uiOffset, const uchar *psMsg, uint uiMsgLen)
{
	...
	pszBuffInOput += sprintf_string((char *)pszBuffInOput, "%04Xh:", uiOffset);

	for (i=0; i<uiMsgLen; i++)
	{
		pszBuffInOput += sprintf_string((char *)pszBuffInOput, " %02X", psMsg[i]);
	}
	for (; i<MAX_CHARS; i++)
	{   // append blank spaces, if needed
		pszBuffInOput += sprintf_string((char *)pszBuffInOput, "   ");
	}
...
void DispAssert(const uchar *pszFileName, ulong ulLineNo)
{
	...
	sprintf_string(szBuff, "FILE:%.11s\nLINE:%ld", szFName, ulLineNo);
...
void GetBaseName(const uchar *pszFullPath, uchar *pszBaseName)
{
	...
	*pszBaseName = 0;
	...
	sprintf_string((char *)pszBaseName, "%s", (char *)(pszTmp+1));
}
