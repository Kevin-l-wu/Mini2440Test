...
int PrintReceipt(uchar ucPrnFlag)
{
	...
	{		
		sprintf_string((char *)szBuff, "%s(%s)", _T("VOID"), _T(glTranConfig[glProcInfo.stTranLog.ucOrgTranType].szLabel));
	}
	else if( glProcInfo.stTranLog.uiStatus & TS_ADJ )
	{
		sprintf_string((char *)szBuff, "%s(%s)", _T(glTranConfig[glProcInfo.stTranLog.ucTranType].szLabel), _T("ADJ"));
	}
	else
	{
		sprintf_string((char *)szBuff, "%s", _T(glTranConfig[glProcInfo.stTranLog.ucTranType].szLabel));
	}
...
int PrnCurAcqTransList(void)
{
	...
	for(iCnt=0; iCnt<MAX_TRANLOG; iCnt++)
	{
		...
		sprintf_string(szTranName, "%.16s", glTranConfig[glProcInfo.stTranLog.ucTranType].szLabel);
		if( glProcInfo.stTranLog.uiStatus & TS_VOID )
		{
			sprintf_string(szTranName, "%.16s", glTranConfig[glProcInfo.stTranLog.ucOrgTranType].szLabel);
			sprintf_string((char *)szBuff, "%s(%s)", _T(szTranName), _T("VOID"));
		}
		else if( glProcInfo.stTranLog.uiStatus & TS_ADJ )
		{
			sprintf_string((char *)szBuff, "%s(%s)", _T(szTranName), _T("ADJ"));
		}
		else
		{
			sprintf_string((char *)szBuff, "%s", _T(szTranName));
		}
...
{	
	...
	for(ucNum=0; ucNum<NumOfReceipt(); ucNum++)
	{
		...
		if( glProcInfo.stTranLog.uiEntryMode & MODE_SWIPE_INPUT )
		{
			sprintf_string(szBuf1, "%s (S)", szBuff);
		}
		else if( glProcInfo.stTranLog.uiEntryMode & MODE_CHIP_INPUT )
		{
			sprintf_string(szBuf1, "%s (C)", szBuff);
		}
		else if( (glProcInfo.stTranLog.uiEntryMode & MODE_FALLBACK_SWIPE) ||
				 (glProcInfo.stTranLog.uiEntryMode & MODE_FALLBACK_MANUAL) )
		{
			sprintf_string(szBuf1, "%s (F)", szBuff);
		}
		else if(glProcInfo.stTranLog.uiEntryMode & MODE_CONTACTLESS)//bug806
		{
		    sprintf_string(szBuf1, "%s (T)", szBuff);
		}
		else
		{
			sprintf_string(szBuf1, "%s (M)", szBuff);
		}
		PrnSetNormal();
		memset(szBuff, 0, sizeof(szBuff));
		sprintf_string((char *)szBuff, "%-23.23s", szBuf1);
		MultiLngPrnStr(szBuff,GUI_ALIGN_LEFT);
		...
		if( ChkIssuerOption(ISSUER_EN_EXPIRY) )
		{
			...
			{
				memset(szBuff, 0, sizeof(szBuff));
				sprintf_string((char *)szBuff, " %2.2s/%2.2s", &glProcInfo.stTranLog.szExpDate[2],
						glProcInfo.stTranLog.szExpDate);
				MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
			}
		}
		...
		PrnSetNormal();
		memset(szBuff, 0, sizeof(szBuff));
		sprintf_string((char *)szBuff, "%-23.23s\n", glProcInfo.stTranLog.szHolderName);
		MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
		...
		{
			sprintf_string((char *)szBuff, "%s", szTranName);
		}
		PrnSetNormal();
		MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
		PrnStr("\n");
		...
		sprintf_string((char *)szBuff, "%06lu          %06lu\n", glCurAcq.ulCurBatchNo, glProcInfo.stTranLog.ulInvoiceNo);
		PrnSetNormal();
		MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
		...
		sprintf_string((char *)szBuf1, "%-22.22s\n", szBuff);
		...
		PrnSetNormal();
		memset(szBuf1, 0, sizeof(szBuf1));
		sprintf_string((char *)szBuf1, "%-14.14s  %-12.12s\n", glProcInfo.stTranLog.szRRN, glProcInfo.stTranLog.szAuthCode);
		MultiLngPrnStr(szBuf1, GUI_ALIGN_LEFT);
		...
		{
			memset(szBuf1, 0, sizeof(szBuf1));
			sprintf_string((char *)szBuf1, "APP:%.16s\n", glProcInfo.stTranLog.szAppLabel);
			MultiLngPrnStr(szBuf1, GUI_ALIGN_LEFT);

			PubBcd2Asc0(glProcInfo.stTranLog.sAID, glProcInfo.stTranLog.ucAidLen, szBuff);
			PubTrimTailChars(szBuff, 'F');
			memset(szBuf1, 0, sizeof(szBuf1));
			sprintf_string((char *)szBuf1, "AID:%.32s\n", szBuff);
			MultiLngPrnStr(szBuf1, GUI_ALIGN_LEFT);

			PubBcd2Asc0(glProcInfo.stTranLog.sAppCrypto, 8, szBuff);
			memset(szBuf1, 0, sizeof(szBuf1));
			sprintf_string((char *)szBuf1, "TC: %.16s\n", szBuff);
			MultiLngPrnStr(szBuf1, GUI_ALIGN_LEFT);
...
void PrnHead(uchar ucFreeFmat)
{
	...
	if (ChkIfTrainMode())
	{
		...
		sprintf_string(szBuff, "%.30s", _T("NOT FOR PAYMENT PROOF"));
		if( szBuff[0]>=0xA0 )
		{
			PrnStr("%.30s\n", szBuff);
		}
...
void PrnHead_T(void)
{
	...
	else
	{
		...
		if (strlen(glSysParam.stEdcInfo.szMerchantAddr)>23)
		{
			...
			PrnSetSmall();
			MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
			sprintf_string(szBuff, "%.23s", glSysParam.stEdcInfo.szMerchantAddr+23);
			MultiLngPrnStr(szBuff, GUI_ALIGN_LEFT);
		}
		else
		{
...
void PrnAmount(const uchar *pszIndent, uchar ucNeedSepLine)
{
    ...
    if (ChkIfNeedTip() && !(glProcInfo.stTranLog.uiStatus & TS_VOID))
	{
		...
		if (ChkIfThermalPrinter())
		{
			sprintf_string((char *)szTempBuff, "%s%s      %s\n", _T(pszIndent), _T("BASE"), szBuff);
		}
		else
		{
			sprintf_string((char *)szTempBuff, "%sBASE      %s\n", pszIndent, szBuff);
		}
		...
		if( !ChkIfZeroAmt(glProcInfo.stTranLog.szTipAmount) )
		{
			...
			{
				sprintf_string((char *)szTempBuff, "%sTIPS      %s\n", pszIndent, szBuff);
			}
		}
		else
		{
			if (ChkIfThermalPrinter())
			{
				sprintf_string((char *)szTempBuff, "%s%s\n", _T(pszIndent), _T("TIPS"), szBuff);
			}
			else
			{
				sprintf_string((char *)szTempBuff, "%sTIPS\n", pszIndent);
			}
		}
		MultiLngPrnStr(szTempBuff, GUI_ALIGN_LEFT);
		...
		if( ucNeedSepLine )
		{
			sprintf_string((char *)szTempBuff, "%s          -----------------\n", pszIndent);
			MultiLngPrnStr(szTempBuff, GUI_ALIGN_LEFT);
		}
		...
		if( !ChkIfZeroAmt(glProcInfo.stTranLog.szTipAmount) )
		{
			...
			{
				sprintf_string((char *)szTempBuff, "%sTOTAL     %s\n", pszIndent, szBuff);
			}
		}
		else
		{
			...
			{
				sprintf_string((char *)szTempBuff, "%sTOTAL\n", pszIndent);
			}
		}
		MultiLngPrnStr(szTempBuff, GUI_ALIGN_LEFT);
...
void PrnStatement(void)
{
	if( glProcInfo.stTranLog.ucTranType==INSTALMENT )
	{
		if( ChkIfBea() )
		{
			MultiLngPrnStr("   I ACCEPT THE T&C OVERLEAF\n", GUI_ALIGN_LEFT);
		}
	}
}
...
int PrintSettle(uchar ucPrnFlag)
{
	...
	if( ucPrnFlag==PRN_NORMAL )
	{
		...
		sprintf_string((char *)glSysCtrl.stRePrnStlInfo.szSettleMsg[glCurAcq.ucIndex], "%s", glProcInfo.szSettleMsg);
...
void PrnParaIssuerSub(uchar ucIssuerKey)
{
	...
	sprintf_string((char *)szBuff, "%lu", glSysParam.stIssuerList[ucCnt].ulFloorLimit);
...
int PrnInstalmentPara(void)
{
	...
	for(ucCnt=0; ucCnt<glSysParam.ucPlanNum; ucCnt++)
	{
		...
		sprintf_string((char *)szBuff, "%lu", glSysParam.stPlanList[ucCnt].ulBottomAmt);
