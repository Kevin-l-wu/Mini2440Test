...
static int GetMsgElement(const char *pszSource, char *pszTag, char *pszValue, int *piLen)
{
	...
	if ((pEqual==NULL) || (pEqual>pEnd))// case: tag without value
	{
		...
		sprintf_string(pszTag, "%.*s", pEnd-pStart, pStart);
		pszValue[0] = 0;
	}
	else
	{
		...
		sprintf_string(pszTag,   "%.*s", pEqual-pStart, pStart);
		sprintf_string(pszValue, "%.*s", pEnd-pEqual-1, pEqual+1);
	}
...
static int PackManagerMsg(const char *pszTag, const void *pszValue, int iValueType)
{
	...
	case MMSG_TYPE_STRING:
		sprintf_string(szBuff, "%.*s", LEN_APPMSG_VALUE, (char *)pszValue);
		break;
	case MMSG_TYPE_UCHAR:
		sprintf_string(szBuff, "%d", *(uchar *)pszValue);
		break;
	case MMSG_TYPE_INT:
		sprintf_string(szBuff, "%d", *(int *)pszValue);
		break;
	case MMSG_TYPE_ULONG:
		sprintf_string(szBuff, "%lu", *(ulong *)pszValue);
	    break;
	default:
	    return ERR_APPMSG_VALUE;
	}
