...
int GenSendPacket(void)
{
	...
	sprintf_string((char *)szBuff, "600%3.3s0000", glCurAcq.szNii);
...
void SaveRecvPackData(void)
{
	...
	if( glProcInfo.stTranLog.ucTranType==LOAD_PARA )
	{
		sprintf_string((char *)glProcInfo.stTranLog.szRspCode, "%.2s", glTMSRecv.szRspCode);
		return;
	}
	...
	sprintf_string((char *)glProcInfo.stTranLog.szDateTime, "%.14s", szLocalTime);

	if( glRecvPack.szAuthCode[0]!=0 )
	{
		sprintf_string((char *)glProcInfo.stTranLog.szAuthCode, "%.6s", glRecvPack.szAuthCode);
	}
	if( glProcInfo.stTranLog.szRRN[0]==0 )
	{
		sprintf_string((char *)glProcInfo.stTranLog.szRRN, "%.12s", glRecvPack.szRRN);
	}
	sprintf_string((char *)glProcInfo.stTranLog.szCondCode,  "%.2s",  glSendPack.szCondCode);
	sprintf_string((char *)glProcInfo.stTranLog.szFrnAmount, "%.12s", glRecvPack.szFrnAmt);
...
int ReferralDial(const uchar *pszPhoneNo)
{
	...
	sprintf_string((char*)szTelNo, "%s%s.", glSysParam.stEdcInfo.szPabx, pszPhoneNo);
...
int AdjustCommParam(void)
{
	...
	if (glCommCfg.ucCommType==CT_MODEM)
	{
		if( glProcInfo.stTranLog.ucTranType==LOAD_PARA )
		{
			sprintf_string((char *)szNewTelNo, "%s.", glSysParam.stEdcInfo.szDownTelNo);
		}
		else if( glProcInfo.stTranLog.ucTranType==SETTLEMENT )
		{
			...
			{
				sprintf_string((char *)szNewTelNo, "%s%s;%s%s.",
			...
			{
				sprintf_string((char *)szNewTelNo, "%s%s.",
						glSysParam.stEdcInfo.szPabx, glCurAcq.stStlPhoneInfo[0].szTelNo);
			}
		...
		sprintf_string((char *)glCommCfg.stPSTNPara.szTelNo, "%s", szNewTelNo);
