...
static void GuiStaticClearScr(const Rect *rect, char IsClearBg);
...
static void GuiStaticDrawLine(int x1, int y1, int x2, int y2, unsigned int color);
...
static int GetInput(const unsigned char *pszPrompt, GUI_TEXT_ATTR stPromptAttr, Rect stPromptRect, unsigned char *pszContent, GUI_TEXT_ATTR stContentAttr, Rect stContentRect, const GUI_INPUTBOX_ATTR *pstType, const Rect *pstButtonRect, GUI_TEXT_ATTR stButtonAttr, int nButtonNo, int timeoutSec) {
    ...
    if (pszPrompt) {
        GuiStaticDrawText(pszPrompt, (void *) &stPromptAttr, stPromptRect.top, stPromptRect.bottom, stPromptRect.left, stPromptRect.right, 0);
    }
    ...
    if ((GUI_INPUT_NUM == pstType->eType || GUI_INPUT_MIX == pstType->eType) && pstType->bSensitive) {
        ...
        GuiStaticDrawText((void *) szSensitive, (void *) &stContentAttr, stContentRect.top, stContentRect.bottom, stContentRect.left, stContentRect.right, 0);
    }
    else {
		...
		GuiStaticDrawText((void *) (szWorkBuff + nOffset), (void *) &stContentAttr, stContentRect.top, stContentRect.bottom, stContentRect.left, stContentRect.right, 0);
    }

    for (i = 0; i < nButtonNo; ++i) { // total buttons on screen, 3 or 16
        GuiStaticDrawBgBox(pstButtonRect[i].left, pstButtonRect[i].top, pstButtonRect[i].right, pstButtonRect[i].bottom, sg_nDefBg);

        if(pstButtonRect[i].nValue != 0) {
            GuiStaticDrawRect(pstButtonRect[i].left, pstButtonRect[i].top, pstButtonRect[i].right, pstButtonRect[i].bottom, sg_nDefColor);
            GuiStaticDrawRect(pstButtonRect[i].left+ 1, pstButtonRect[i].top+ 1, pstButtonRect[i].right-1, pstButtonRect[i].bottom-1, sg_nDefColor);
        }
        ...
        if(KEY0 <= pstButtonRect[i].nValue && pstButtonRect[i].nValue <= KEY9) {
            ...
            GuiStaticDrawText((void *)szCH, (void *)&stButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
        }
        else
        {
            switch (pstButtonRect[i].nValue) {
                case KEYENTER:
                    GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYENTER), (void *) &stButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
                    break;
                case KEYCANCEL:
                    GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYCANCEL), (void *) &stButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
                    break;
                case KEYCLEAR:
                    GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYCLEAR), (void *) &stButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
                    break;
                case KEYALPHA:
                    GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYALPHA), (void *) &stButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
                    break;
            }
        }
    }
    ...
    while (1) {
        ...
        if (ucTimerFlag && 0 == TimerCheck(GUI_TIMER_INPUT_TEMP) &&  // Modified by Kim_LinHB 2014-09-03 v0.5
            ...
            if (sg_uiArabicFileID != AR_OPENFILE_ERROR) // Added by Kim_LinHB 2014-09-01 v0.4
                GuiStaticClearScr(&stContentRect, 1);
#endif
            GuiStaticDrawText((void *) szSensitive, (void *) &stContentAttr, stContentRect.top, stContentRect.bottom, stContentRect.left, stContentRect.right, 0);
            ucTimerFlag = FALSE;
        }
        ...
        if (KEYCANCEL == iKey) {
            ...
            if (bOKToReturn) {
                if (pszContent)
                    sprintf_string((char *) pszContent, "%s", szWorkBuff);
                return GUI_OK;
            }
        }
...
static int GetInputTime(Rect stCurrTimeRect, const Rect *pstTimeRect, GUI_TEXT_ATTR stTimeAttr, unsigned char *pszTime, const Rect *pstButtonRect, unsigned char nButtonsNo, GUI_TEXT_ATTR stButtonAttr, unsigned char isTime, int timeoutSec) {
    ...
    GetTime(sTimeBCD);
    sprintf_string(szSettingTime, "20%02X%02X%02X%02X%02X%02X", sTimeBCD[0], sTimeBCD[1], sTimeBCD[2], sTimeBCD[3], sTimeBCD[4], sTimeBCD[5]);

    if (!isTime) {
        sprintf_string(szCurrTime, "20%02X-%02X-%02X", sTimeBCD[0], sTimeBCD[1], sTimeBCD[2]);
        sprintf_string(szBuff[0], "20%02X", sTimeBCD[0]);
        sprintf_string(szBuff[1], "%02X", sTimeBCD[1]);
        sprintf_string(szBuff[2], "%02X", sTimeBCD[2]);
    }
    else {
        sprintf_string(szCurrTime, "%02X:%02X", sTimeBCD[3], sTimeBCD[4]);
        sprintf_string(szBuff[0], "%02X", sTimeBCD[3]);
        sprintf_string(szBuff[1], "%02X", sTimeBCD[4]);
    }
    GuiStaticDrawText(szCurrTime, (void *) &stTimeAttr, stCurrTimeRect.top, stCurrTimeRect.bottom, stCurrTimeRect.left, stCurrTimeRect.right, 0);
...

    {
        ...
        for (j = 0; j < nTotal; ++j) {
            if ( j == ucSelected) {
                GuiStaticDrawText((void *) szBuff[j], (void *) &stReversal, pstTimeRect[j].top, pstTimeRect[j].bottom, pstTimeRect[j].left, pstTimeRect[j].right, 1);
            }
            else {
                GuiStaticDrawText((void *) szBuff[j], (void *) &stTimeAttr, pstTimeRect[j].top, pstTimeRect[j].bottom, pstTimeRect[j].left, pstTimeRect[j].right, 1);
            }
        }
    }
    ...
    while (1) {
        ...
        if (KEYCANCEL == iKey) {
			...
			char toSwitch = 1;
			GuiStaticClearScr(&pstTimeRect[ucSelected], 1);
			GuiStaticDrawBgBox(pstTimeRect[ucSelected].left, pstTimeRect[ucSelected].top, pstTimeRect[ucSelected].right, pstTimeRect[ucSelected].bottom, sg_nDefBg);
			GuiStaticDrawText((void *)szBuff[ucSelected], (void *)&stTimeAttr,
			...
			if (toSwitch && nCurrLen >= nMax[ucSelected]) {
				...
				if (0 == IsValidTime(szTmpTime, szSetTimeFormat)) {
					...
					nCurrLen = nMax[ucSelected];
					sprintf_string(szBuff[ucSelected], "%.*s", nMax[ucSelected], szSettingTime + nOffset[ucSelected]);
				}
			}
		}
...
static void GuiStaticClearScr(const Rect *rect, char IsClearBg) {
	...
	if(!rect) {
		...
		if(IsClearBg)
		GuiStaticDrawBgBox(stTmp.left, stTmp.top, stTmp.right, stTmp.bottom, sg_nDefBg);
...
static void GuiStaticDrawText(const void *vRes, const void *vAttr, int top, int bottom, int left, int right, char isButton) {
	...
	if (stTextAttr.eStyle & GUI_FONT_OPAQUE) {
		if (stTextAttr.eStyle & GUI_FONT_REVERSAL) {
			GuiStaticDrawBgBox(left, top, right, bottom, _GetReverseColor(sg_nDefBg));
			CLcdSetFgColor(_GetReverseColor(sg_nDefColor));
		}
		else {
			GuiStaticDrawBgBox(left, top, right, bottom, sg_nDefBg);
	...
	do {
		...
		width = GetStrPix((unsigned char *) szOneLine, &sgFontS[stTextAttr.eFontSize], &sgFontM[stTextAttr.eFontSize], 1);
		if(width > right - left){
			int end = right - left;
			end /= sgFontS[stTextAttr.eFontSize].Width;
			if(!isButton)
				memmove(szOneLine, &szOneLine[strlen(szOneLine) - end], end);
			szOneLine[end] = 0;
		}
		nLeft = left;
	...
	if(1 == TouchScreenRead(&stPoint, 1) && 100 == stPoint.pressure) {
		if(area && _IsIncluded(stPoint.x, stPoint.y, area->top, area->bottom, area->left, area->right) && sgPoint.x != stPoint.x && sgPoint.y != stPoint.y){
			if(_IsIncluded(sgPoint.x, sgPoint.y,  area->top, area->bottom, area->left, area->right)){
				GuiStaticDrawLine(sgPoint.x, sgPoint.y, stPoint.x, stPoint.y, sg_nDefColor);
				GuiStaticDrawLine(sgPoint.x, sgPoint.y +1 , stPoint.x+1, stPoint.y +1 , sg_nDefColor);
				GuiStaticDrawLine(sgPoint.x+1, sgPoint.y, stPoint.x+1, stPoint.y, sg_nDefColor);
			}
...
// continue to updating UI & state
            ...
            if(pRect) {
                if(memcmp(pRect, &sg_rect_bak, sizeof(Rect))) { // moving
                    GuiStaticDrawBgBox(pRect->left, pRect->top, pRect->right, pRect->bottom, _GetReverseColor(sg_nDefBg));
                    GuiStaticDrawBgBox(sg_rect_bak.left, sg_rect_bak.top, sg_rect_bak.right, sg_rect_bak.bottom, sg_nDefBg);
            ...
            else
            {
				if(!_IsIncluded(stPoint.x, stPoint.y, sg_rect_bak.top, sg_rect_bak.bottom, sg_rect_bak.left, sg_rect_bak.right)) {
					...
					GuiStaticDrawBgBox(sg_rect_bak.left, sg_rect_bak.top, sg_rect_bak.right, sg_rect_bak.bottom, _GetReverseColor(sg_nDefBg));
				}
            }
...
// continue to updating UI & state
		...
		case 4:
			if(_IsIncluded(sg_pt.x, sg_pt.y, sg_rect_bak.top, sg_rect_bak.bottom, sg_rect_bak.left, sg_rect_bak.right)){
				...
				sg_pt_state = 0;
				if(pRect) {
					GuiStaticDrawBgBox(pRect->left, pRect->top, pRect->right, pRect->bottom, sg_nDefBg);
				}
				else {
					GuiStaticDrawBgBox(sg_rect_bak.left, sg_rect_bak.top, sg_rect_bak.right, sg_rect_bak.bottom, sg_nDefBg);
					memset(&sg_rect_bak, 0, sizeof(Rect));
...
static int GetMenuItem(const GUI_MENU *pstMenu, enum MENUSTYLE eMode, Rect stMenuRect, const Rect *pstButtonRect, unsigned char nButtonNo, const GUI_TEXT_ATTR *pstButtonAttr, int timeoutSec, int *piSelected) {
    ...
    GuiStaticClearScr(&stMenuRect, 1);
    memset(stItemsRect, 0, sizeof(stItemsRect));

//title
    if (pstMenu->szTitle[0] != 0) {
        GuiStaticDrawText(pstMenu->szTitle, (void *) &pstMenu->stTitleAttr, sgTitleArea.top, sgTitleArea.bottom, sgTitleArea.left, sgTitleArea.right, 0);
    }
    ...
    while (1) {
		...
		if (pstButtonRect && pstButtonAttr && nButtonNo > 0) {
			for (i = 0; i < nButtonNo; ++i) {
				GuiStaticClearScr(&pstButtonRect[i], 0);
				GuiStaticDrawBgBox(pstButtonRect[i].left, pstButtonRect[i].top, pstButtonRect[i].right, pstButtonRect[i].bottom, sg_nDefBg);
				if(pstButtonRect[i].nValue != 0) {
					GuiStaticDrawRect(pstButtonRect[i].left, pstButtonRect[i].top, pstButtonRect[i].right, pstButtonRect[i].bottom, sg_nDefColor);
					GuiStaticDrawRect(pstButtonRect[i].left+ 1, pstButtonRect[i].top+ 1, pstButtonRect[i].right-1, pstButtonRect[i].bottom-1, sg_nDefColor);
				...
				switch (pstButtonRect[i].nValue) {
				case KEYENTER:
					GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYENTER), (void *) pstButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
					break;
				case KEYCANCEL:
					GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYCANCEL), (void *) pstButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
					break;
				case GUI_KEYPREV: 
					GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYUP), (void *) pstButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
					break;
				case GUI_KEYNEXT: 
					GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYDOWN), (void *) pstButtonAttr, pstButtonRect[i].top, pstButtonRect[i].bottom, pstButtonRect[i].left, pstButtonRect[i].right, 1);
					break;
				}
			}
		}
...
static int GetInfoPage(const GUI_PAGE *pstPage, Rect stInfoRect, const Rect *pstButtonRect, unsigned char nButtonNo, const GUI_TEXT_ATTR *pstButtonAttr, unsigned char isMultiChapters, int timeoutSec) {
    ...
    GuiStaticClearScr(&stInfoRect, 1);

//title
    if (pstPage->szTitle[0] != 0) {
        GuiStaticDrawText(pstPage->szTitle, (void *) &pstPage->stTitleAttr, sgTitleArea.top, sgTitleArea.bottom, sgTitleArea.left, sgTitleArea.right, 0);
    }
...
// Modified by Kim v0.5
	...
	if(isMultiChapters){
		...
		stRectMap[2].no = 1;
		GuiStaticDrawBgBox(sgInfoPageMenu.left, sgInfoPageMenu.top, sgInfoPageMenu.right, sgInfoPageMenu.bottom, sg_nDefBg);
		if(sg_hasTouchScreen) {
			GuiStaticDrawRect(sgInfoPageMenu.left, sgInfoPageMenu.top, sgInfoPageMenu.right, sgInfoPageMenu.bottom, sg_nDefColor);
			GuiStaticDrawRect(sgInfoPageMenu.left+ 1, sgInfoPageMenu.top+ 1, sgInfoPageMenu.right-1, sgInfoPageMenu.bottom-1, sg_nDefColor);
		}
		GuiStaticDrawText((void *) GetKeyValuebyIndex(KEYMENU), (void *) pstButtonAttr, sgInfoPageMenu.top, sgInfoPageMenu.bottom, sgInfoPageMenu.left, sgInfoPageMenu.right, 1);
    ...
    while (1) {
		...
		if(isPopUp){
			for (i = 0; i < 2; ++i) { // cancel & ok(next) & clear(previous)
				GuiStaticDrawBgBox(sgInfoPageMenuOpt[i].left, sgInfoPageMenuOpt[i].top, sgInfoPageMenuOpt[i].right, sgInfoPageMenuOpt[i].bottom, sg_nDefBg);
				if(sg_hasTouchScreen) {
					if(sgInfoPageMenuOpt[i].nValue != 0) {
						GuiStaticDrawRect(sgInfoPageMenuOpt[i].left, sgInfoPageMenuOpt[i].top, sgInfoPageMenuOpt[i].right, sgInfoPageMenuOpt[i].bottom, sg_nDefColor);
						GuiStaticDrawRect(sgInfoPageMenuOpt[i].left+ 1, sgInfoPageMenuOpt[i].top+ 1, sgInfoPageMenuOpt[i].right-1, sgInfoPageMenuOpt[i].bottom-1, sg_nDefColor);
					}
				}
				// Modified by Kim_LinHB 20140916 v0.8 bug522
				switch (sgInfoPageMenuOpt[i].nValue) {
				case GUI_KEYPREV:
					GuiStaticDrawText((void *) GetKeyValuebyIndex(GUI_KEYPREV), (void *) pstButtonAttr, sgInfoPageMenuOpt[i].top, sgInfoPageMenuOpt[i].bottom, sgInfoPageMenuOpt[i].left, sgInfoPageMenuOpt[i].right, 1);
					break;
				case GUI_KEYNEXT:
					GuiStaticDrawText((void *) GetKeyValuebyIndex(GUI_KEYNEXT), (void *) pstButtonAttr, sgInfoPageMenuOpt[i].top, sgInfoPageMenuOpt[i].bottom, sgInfoPageMenuOpt[i].left, sgInfoPageMenuOpt[i].right, 1);
					break;
				}
			}
		}
    ...
    *piScrItemNum = 0;
    ...
    for (i = iCurItem; i < (int) pstMenu->nSize; ++i) {
        ...
        if (eMode & GUI_MENU_MANUAL_INDEX) {
            sprintf_string(buf, "%d.%s", pstMenu->pstContent[i].nValue, pstMenu->pstContent[i].szText);
        }
        else {
            sprintf_string(buf, "%d.%s", iNumIndex, pstMenu->pstContent[i].szText);
        }
...
//GuiStaticClearScr(&stForClean);
                GuiStaticDrawText(menu_text[i], (void *) &stTextAttr, pstItemsRect[i].top, pstItemsRect[i].bottom, pstItemsRect[i].left, pstItemsRect[i].right, 1);
            }
            else {
//GuiStaticClearScr(&stForClean);
				GuiStaticClearScr(&pstItemsRect[i], 1);
				GuiStaticDrawBgBox(pstItemsRect[i].left, pstItemsRect[i].top, pstItemsRect[i].right, pstItemsRect[i].bottom, sg_nDefBg);

                GuiStaticDrawText(menu_text[i], (void *) &pstMenu->stItemAttr, pstItemsRect[i].top, pstItemsRect[i].bottom, pstItemsRect[i].left, pstItemsRect[i].right, 1);
            }
            ++i;
        }
    }
...
static void PrepareScrPage(const GUI_PAGE *pstPage, Rect stPageRect, int iStartIndex, int iLineInPage) {
    ...
    for (i = iStartIndex; i < (int) pstPage->nLine && i < (iStartIndex + iLineInPage); ++i) {
        ...
        GuiStaticDrawText(pstPage->pstContent[i].szLine, (void *) &pstPage->pstContent[i].stLineAttr, top, top + iHeight, stPageRect.left, stPageRect.right, 0);
        top += iHeight;
    }
...
unsigned long Asc2Long(const unsigned char *psString, unsigned int uiLength) {
    ...
    sprintf_string((char *) szBuff, "%.*s", uiLength <= 15 ? uiLength : 15, psString);
...
void AdjustDateTime(unsigned char *pszYYMMDDhhmmss, short nOffset, unsigned char ucMode) {
    ...
    sprintf_string(szTmp, "%0*d", 2, nCurrValue);
