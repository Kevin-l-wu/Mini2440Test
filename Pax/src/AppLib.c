...
void PubLong2Bcd(ulong ulSource, uint uiTCnt, uchar *psTarget)
{
    ...
    sprintf_string((char*)szFmt, "%%0%dlu", uiTCnt*2);
    sprintf_string((char*)szBuf, (char *)szFmt, ulSource);
    if( psTarget != NULL )
    {
...
void PubAscMul(const uchar *pszFaciend, const uchar *pszMultiplier, uchar *pszProduct)
{
	...
	sprintf_string((char *)szBuff, "%0*ld", uiFaciLen+uiMulLen, 0L);
	...
	if( szBuff[0]==0 )
	{
		sprintf_string((char *)szBuff, "0");
	}
	if( pszProduct!=NULL )
	{
		sprintf_string((char *)pszProduct, "%s", szBuff);
	}
}
...
void PubAscAdd(const uchar *psAddend1, const uchar *psAddend2, uint uiLen, uchar *pszSum)
{
	...
	*pszResult = 0;
	...
	if( pszSum!=NULL )
	{
		sprintf_string((char *)pszSum, "%s", szBuff);
	}
}
...
void PubAscSub(const uchar *psMinuend, const uchar *psSubtrahend, uint uiLen, uchar *pszResult)
{
	...
	*pszOut = 0;
	...
	if( pszResult!=NULL )
	{
		sprintf_string((char *)pszResult, "%s", szBuff);
	}
}
...
void PubConvAmount(const uchar *pszPrefix, const uchar *pszIn, uchar ucDeciPos, uchar ucIgnoreDigit, uchar *pszOut, uchar ucMisc)
{
	...
	sprintf_string((char *)szBuff, "%.40s", pszIn);
	...
	{
		pWr += sprintf_string((char *)pWr, "%.3s ", pszPrefix);
	...
	if (ucDeciPos!=0)
	{
		sprintf_string((char *)szTemp, "%.*s", ucDeciPos, pRead);
		PubAddHeadChars(szTemp, ucDeciPos, '0');
		sprintf_string((char *)pWr, ".%s", szTemp);
		//		sprintf_string((char *)pWr, ".%0*s", ucDeciPos, pRead);
		pWr += (ucDeciPos+1);
	}
	else
	{
...
void PubLong2Day(ulong ulDay,uchar *pszYYYYMMDD)  
{
	...
	sprintf_string(&pszYYYYMMDD[0],"%04lu",ulYear+STAR_YEAR);
	sprintf_string(&pszYYYYMMDD[4],"%02lu",ulMonth);
	sprintf_string(&pszYYYYMMDD[6],"%02lu",ulDay);
	pszYYYYMMDD[8]=0;
} 
...
void PubLong2Time(ulong ulSecond,uchar *pszYYYYMMDDhhmmss)   
{
	...
	sprintf_string(&pszYYYYMMDDhhmmss[0],"%04lu",ulYear+STAR_YEAR);
	sprintf_string(&pszYYYYMMDDhhmmss[4],"%02lu",ulMonth);
	sprintf_string(&pszYYYYMMDDhhmmss[6],"%02lu",ulDay);
	sprintf_string(&pszYYYYMMDDhhmmss[8],"%02lu",ulDaySecond/3600);
	ulDaySecond=ulDaySecond%3600;
	sprintf_string(&pszYYYYMMDDhhmmss[10],"%02lu",ulDaySecond/60);
	ulDaySecond=ulDaySecond%60;
	sprintf_string(&pszYYYYMMDDhhmmss[12],"%02lu",ulDaySecond);
	//	sprintf_string(&pszYYYYMMDDhhmmss[14],"%02lu",ulWeek); //no need week yet
...
{//YYYYMMDDhhmmss format
	...
	if (psCalMode!=NULL)
	{
		...
		case 'Y':
			...
			sprintf_string(pszDateTimeOut,"%04lu",ulYear+ulInterval);
		...
		case 'M':
			...
			sprintf_string(pszDateTimeOut,"%04lu",ulYear);
			sprintf_string(pszDateTimeOut+4,"%02lu",ulMonth);
...
uchar ScrSend(const uchar *psData, ushort nDataLen, ushort nStartLine)
{
	...
	while(1)
	{
		...
		if (nDataLen - nCurrLen > 21)
		{
			...
			sprintf_string(szBuff, "%21.21s", psData+nCurrLen);
		...
		{   
			...
			sprintf_string(szBuff, "%.*s", nDataLen - nCurrLen, psData+nCurrLen);
			Gui_DrawText(szBuff, gl_stLeftAttr, 0, nCurrLine%6+25);
...
//	}
	...
	if (pszTitle != NULL)
	{
		sprintf_string(szTitle, "%.*s", 32, pszTitle);
	}
...
//				UnitSend(psData+usCurrLen, usUnitLen, ucDevice, ucMode, 0);
		...
		while(1)
		{
			if (nDataLen-nCurrLen < nUnitLen/3)
			{
				for (iCnt=0; iCnt < nDataLen-nCurrLen; iCnt++)
					sprintf_string(sDataBuff+iCnt*3, "%02x ", psData[nCurrLen+iCnt]);
	...
	if (ucMode&ISO_MODE)
	{
		...
		nCurrLen += uiFieldLen;
		sprintf_string(sDataBuff, "[1]=");
		nFixLen = strlen(sDataBuff);
		for (iCnt = 0; iCnt < (int)uiFieldLen; iCnt++)
			sprintf_string(sDataBuff+nFixLen+iCnt*3, "%02x ", sBitmapBuf[iCnt]);	
		...
		for (iFieldCnt=1; iFieldCnt<(int)(uiBitmap*8); iFieldCnt++)
		{
			if( sBitmapBuf[iFieldCnt/8] & (0x80>>iFieldCnt%8)) 
			{
				...
				if( iRet<0 || uiFieldLen > (pstDataAttr+1+iFieldCnt)->uiLength || uiFieldLen+nCurrLen > nDataLen||
					uiFieldLen <= 0)
				{
					sprintf_string(sDataBuff, "field %d unpack error", iFieldCnt+1);
...
//					return 0xff;
				...
				if ((pstDataAttr+1+iFieldCnt)->eLengthAttr != Attr_fix)
				{
					...
					sprintf_string(sDataBuff, "[%d]=(%u)", iFieldCnt+1, nTmpLen);
				}
				else
				{
					nTmpLen = (pstDataAttr+1+iFieldCnt)->uiLength;
					sprintf_string(sDataBuff, "[%d]=", iFieldCnt+1);
					ucLengthValue = 0;
				}
				nFixLen = strlen(sDataBuff);
				...
				{											
					for (iCnt=0; iCnt< nTmpLen; iCnt++)
						sprintf_string(sDataBuff+nFixLen+iCnt*3, "%02x ", sFieldBuff[iCnt+ucLengthValue]);
					nSendLen = nFixLen+nTmpLen*3;
				}
				else
				{					
		...
		if (ucOverFlag == 0)
		{		
			sprintf_string(sDataBuff, "pack ok!");
			UnitSend(sDataBuff, (ushort)(strlen(sDataBuff)), (uchar)(ucDevice|END_FLAG), ucMode, nCurrLine);
		}
		pstDataAttr = NULL;
	}
	...
	if (ucMode&TLV_MODE)
	{
		...
		while (nCurrLen < nDataLen)
		{
			...
			{
				sprintf_string(sDataBuff, "Tag:%02x", psData[nCurrLen]);
				nCurrLen ++;
...
// 					sprintf_string(sDataBuff+usFixLen, "%02x ", psData[usCurrLen]);
// 					usCurrLen ++;
// 				}while((psData[usCurrLen-1]&0x80));
				nFixLen = strlen(sDataBuff);
				sprintf_string(sDataBuff+nFixLen, "%02x ", psData[nCurrLen]);
				nCurrLen++;
				nSendLen = strlen(sDataBuff);
				UnitSend(sDataBuff, nSendLen, ucDevice, ucMode, nCurrLine);
				nCurrLine += (nSendLen)/21+(nSendLen%21?1:0);
			...
			if (psData[nCurrLen]&0x80)//n bytes length
			{
				...
				nCurrLen ++;
				sprintf_string(sDataBuff, "  L:");
				nFixLen = strlen(sDataBuff);
				for (iCnt = 0; iCnt < (int)uiFieldLen; ++iCnt)
				{
					nTmpLen += psData[nCurrLen+iCnt]<<(8*(iCnt-1));
					sprintf_string(sDataBuff+nFixLen+iCnt*3, "%02x ", psData[nCurrLen+iCnt]);
				}
				nCurrLen += uiFieldLen;
				nSendLen = strlen(sDataBuff);
				UnitSend(sDataBuff, nSendLen, ucDevice, ucMode, nCurrLine);
			...
			{
				sprintf_string(sDataBuff, "  L:%02x", psData[nCurrLen]);
			...
			{
				sprintf_string(sDataBuff, "TLV fail!");
				UnitSend(sDataBuff, (ushort)(strlen(sDataBuff)), (uchar)(ucDevice|END_FLAG), ucMode, nCurrLine);
				ucOverFlag = 1;
				break;
			}
			sprintf_string(sDataBuff, "  V:");
			nFixLen = strlen(sDataBuff);
			for (iCnt = 0; iCnt < nTmpLen; iCnt++)
				sprintf_string(sDataBuff+nFixLen+iCnt*3, "%02x ", psData[nCurrLen+iCnt]);
			nCurrLen += nTmpLen;
		...
		if (ucOverFlag == 0)
		{
			sprintf_string(sDataBuff, "TLV ok!");
			UnitSend(sDataBuff, (ushort)(strlen(sDataBuff)), (uchar)(ucDevice|END_FLAG), ucMode, nCurrLine);		
		}
	}
	if (ucOverFlag)
